...
async function returnsPanel(){
 try{
  ...
  const rows=delivered.map(o=>{
   ...
   const form=(!r&&win.eligible&&!locked)?`<div id="${formId}" style="display:none;margin:-6px 0 14px;padding:14px;background:#fffaf7;border:1px solid #eaded8;border-radius:9px"><div class="form"><label><b>Return reason</b></label><select id="return-reason-${o.id}" onchange="syncReturnOption(${o.id})"><option value="">Select reason</option><option>Size too small</option><option>Size too large</option><option>Colour / Style issue</option><option>Damaged / Defective</option><option>Other</option></select><label><b>What do you want?</b></label><select id="return-type-${o.id}" onchange="syncReturnOption(${o.id})"><option value="REPLACEMENT">Replacement (standard policy)</option><option value="EXCHANGE">Exchange / Replace with another size or colour</option><option value="RETURN_REFUND">Return for refund (admin approval required)</option></select><small id="return-rule-${o.id}">Replacement is the standard option. A return for refund starts only after admin approval and product inspection.</small><div id="return-extra-${o.id}" style="display:none;grid-template-columns:1fr 1fr;gap:8px"><input id="return-size-${o.id}" placeholder="New size (optional)"><input id="return-color-${o.id}" placeholder="New colour (optional)"></div><button class="gold" type="button" onclick="submitReturn(${o.id})">Submit Return Request</button><button type="button" onclick="toggleReturnForm(${o.id})">Cancel</button></div></div>`:'';
...
async function sendOtp(){
 const phoneEl=document.getElementById('rphone'),hint=document.getElementById('otpHint'),phone=(phoneEl?.value||'').replace(/\D/g,''),name=document.getElementById('rn')?.value||'',email=document.getElementById('re')?.value||'',password=document.getElementById('rp')?.value||'',whatsappMarketingOptIn=!!document.getElementById('rWhatsappMarketing')?.checked;
 ...
 try{
  ...
  const verified=await api('/api/auth/register-msg91',{method:'POST',body:{name,email,password,phone,accessToken,whatsapp_marketing_opt_in:whatsappMarketingOptIn}});if(flowId!==__msg91FlowId)return;session(verified);closeM();toast('✓ Account created');
...
// Courier details are rendered inside the existing tracking modal, so the layout stays responsive.
async function track(id){
 try{
  const o=await api('/api/orders/'+id);let i=stages.indexOf(o.status);if(i<0)i=0;
  const safeTrackingUrl=o.tracking_url&&/^https?:\/\//i.test(String(o.tracking_url))?String(o.tracking_url):'';
  const courier=o.tracking_number?`<div style="margin:14px 0;padding:12px;border:1px solid #d8c7cc;border-radius:9px;background:#fffafc;overflow-wrap:anywhere"><b>📦 Courier tracking</b><div style="margin-top:7px">Courier: <b>${esc(o.courier_name||'Courier partner')}</b></div><div>AWB / Tracking number: <b>${esc(o.tracking_number)}</b></div>${o.dispatched_at?`<div>Dispatched: ${new Date(o.dispatched_at).toLocaleString('en-IN')}</div>`:''}${safeTrackingUrl?`<a class="gold" style="display:inline-block;margin-top:9px;padding:9px 13px;border-radius:7px;text-decoration:none" href="${esc(safeTrackingUrl)}" target="_blank" rel="noopener noreferrer">Track on courier website ↗</a>`:''}</div>`:'<p><small>Courier and AWB details will appear here after dispatch.</small></p>';
  openM(`<h2>🚚 Track Order #${o.id}</h2><p><b>Current status:</b> ${esc(String(o.status).replaceAll('_',' '))}</p><div class="track">${stages.map((s,n)=>`<div class="step ${n<=i?'active':''}"><div class="dot"></div>${s.replaceAll('_',' ')}</div>`).join('')}</div>${courier}<p><b>Delivering to:</b> ${esc(o.address)}</p><p><b>Payment:</b> ${esc(String(o.payment_status||'PENDING').replaceAll('_',' '))}</p>${o.refund_status?`<p><b>Refund:</b> ${esc(String(o.refund_status).replaceAll('_',' '))}</p>`:''}${o.dispute_status?`<p><b>Payment dispute:</b> ${esc(String(o.dispute_status).replaceAll('_',' '))}</p>`:''}<div class="total">₹${Number(o.total).toLocaleString('en-IN')}</div><button class="account-action" type="button" onclick="orders()">← My Orders</button>`);
 }catch(e){alert('Tracking could not load: '+e.message)}
}
 ...
 try{
  if(type==='products'){
   ...
   if(type==='customers'){
    ...
    const rows=list.map(x=>`<tr><td>#${x.id}<br><small>Ordered: ${new Date(x.created_at||'').toLocaleString('en-IN')}</small>${x.status==='DELIVERED'&&x.updated_at?`<br><small>Delivered: ${new Date(x.updated_at).toLocaleString('en-IN')}</small>`:''}</td><td>${esc(x.name||'Customer')}</td><td>₹${Number(x.total).toLocaleString('en-IN')}</td><td>${esc(x.payment_status||'')}</td><td>${esc(x.status||'')}</td><td>${orderPrintReady(x)?`<button type="button" onclick="printOrderDocument(${x.id},'shipping')">🏷️ Label</button> <button class="gold" type="button" onclick="printOrderDocument(${x.id},'invoice')">🧾 Bill</button>`:'<small>Not ready</small>'}</td></tr>`).join('')||'<tr><td colspan="6">No orders yet.</td></tr>';
