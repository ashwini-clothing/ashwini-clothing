...
window.searchByPhoto=async function(input){const file=input?.files?.[0]||input;if(!file)return;const grid=document.getElementById('grid'),title=document.getElementById('resultTitle'),count=document.getElementById('resultCount');try{closeLensExperience?.();if(title)title.textContent='AI Visual Search';if(count)count.textContent='(analyzing photo…)';if(grid)grid.innerHTML='<div class="visual-search-loading"><span></span><b>AI photo ko analyze kar raha hai…</b><small>Matching products dhoondhe ja rahe hain</small></div>';document.getElementById('products')?.scrollIntoView({behavior:'smooth',block:'start'});const imageData=await visualSearchImageData(file);let products=[],summary='',usedFree=false;try{const data=await api('/api/visual-search',{method:'POST',body:{imageData}});products=Array.isArray(data.results)?data.results:[];summary=String(data.analysis?.summary||data.analysis?.garment_type||'Related products')}catch{usedFree=true;products=await freeVisualSearch(imageData)}if(title)title.textContent=usedFree?'Free Visual Match':'AI Match: '+summary;if(count)count.textContent=`(${products.length} results)`;if(grid)grid.innerHTML=products.length?products.map(productCard).join(''):'<div class="visual-search-empty"><b>No close product match found.</b></div>';toast(usedFree?'✓ Free matching complete':'✓ AI matching complete')}catch(e){if(grid)grid.innerHTML=`<div class="visual-search-empty"><b>Photo search could not start.</b><small>${esc(e.message||'Please try again.')}</small></div>`;toast(e.message||'AI photo search failed')}finally{if(input?.value!==undefined)input.value=''}};
