...
async function visualSearchImageData(file){
 if(!file||!String(file.type||'').startsWith('image/'))throw Error('Please choose a valid photo.');
 if(file.size>12*1024*1024)throw Error('Please choose a photo under 12 MB.');
 const source=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=()=>reject(Error('Photo could not be read.'));r.readAsDataURL(file)});
 const img=await new Promise((resolve,reject)=>{const x=new Image();x.onload=()=>resolve(x);x.onerror=()=>reject(Error('Photo could not be opened.'));x.src=source});
...
window.searchByPhoto=async function(input){
 ...
 if(!file)return;
 const grid=document.getElementById('grid'),title=document.getElementById('resultTitle'),count=document.getElementById('resultCount');
 try{
  closeLensExperience?.();
  if(title)title.textContent='AI Visual Search';
  if(count)count.textContent='(analyzing photo…)';
  ...
  if(grid)grid.innerHTML=`<div class="visual-search-empty"><b>Photo search could not start.</b><small>${esc(e.message||'Please try again.')}</small><button class="gold" type="button" onclick="document.getElementById('photoUploadInput')?.click()">Choose another photo</button></div>`;
  toast(e.message||'AI photo search failed');
 }finally{if(input?.value!==undefined)input.value=''}
};
...
function auth(prefill='',notice=''){
 ...
 <div class="form">
   ...
   <div class="login-notice" role="status">${notice?esc(notice):''}</div>
...
async function loadMsg91Sdk(){if(typeof window.initSendOTP==='function')return;if(__msg91SdkReady)return __msg91SdkReady;__msg91SdkReady=new Promise((resolve,reject)=>{const sources=['https://verify.msg91.com/otp-provider.js','https://verify.phone91.com/otp-provider.js'];let i=0;const next=()=>{if(i>=sources.length){__msg91SdkReady=null;reject(Error('MSG91 OTP service could not be loaded.'));return}const s=document.createElement('script');s.src=sources[i++];s.async=true;s.onload=()=>typeof window.initSendOTP==='function'?resolve():next();s.onerror=next;document.head.appendChild(s)};next()});return __msg91SdkReady}
function warmMsg91(){if(__msg91Warmup)return __msg91Warmup;return __msg91Warmup=Promise.all([api('/api/auth/msg91-config'),loadMsg91Sdk()]).catch(e=>{__msg91Warmup=null;throw e})}
function msg91AccessToken(data){const seen=new Set(),walk=v=>{if(!v||seen.has(v))return '';if(typeof v==='string')return v.length>30?v:'';if(typeof v!=='object')return '';seen.add(v);for(const k of ['accessToken','access_token','access-token','jwt','token','authToken'])if(typeof v[k]==='string'&&v[k].length>30)return v[k];for(const x of Object.values(v)){const found=walk(x);if(found)return found}return ''};return walk(data)}
async function openMsg91Verification(phone){const [cfg]=await warmMsg91();const own=document.getElementById('modal');if(own){own.style.display='none';own.setAttribute('aria-hidden','true');document.body.style.overflow=''}return new Promise((resolve,reject)=>{window.initSendOTP({widgetId:cfg.widgetId,tokenAuth:cfg.tokenAuth,identifier:'91'+phone,success:data=>{const token=msg91AccessToken(data);token?resolve(token):reject(Error('MSG91 verification token was not returned.'))},failure:e=>reject(Error(e?.message||e?.error||'MSG91 verification failed.'))})})}
async function continueCustomerLogin(){
 ...
 if(isMobileIdentifier(identifier)){
  const phone=identifier.replace(/\D/g,''),flow=++__msg91FlowId;
  openM(`<div class="amazon-login-wrap"><h2>Secure mobile verification</h2><p>+91 ${esc(phone)}</p><small>Opening MSG91 secure OTP screen…</small><button class="linkbtn" onclick="auth('${esc(phone)}')">← Back</button></div>`);
  try{const accessToken=await openMsg91Verification(phone);if(flow!==__msg91FlowId)return;const d=await api('/api/auth/verify-msg91-login',{method:'POST',body:{identifier:phone,accessToken}});session(d);closeM();toast('✓ Signed in')}catch(e){if(flow===__msg91FlowId)auth(phone,e.message||'Mobile verification failed')}
  return;
 }
 try{
...
function showRegisterPanel(){
 ...
 <button type="button" class="gold" onclick="sendOtp()">Verify mobile & create account</button><small id="otpHint">OTP will be sent and verified securely by MSG91.</small><button type="button" class="linkbtn" onclick="auth()">← Back to Sign in</button></div>`);
}
...
async function sendOtp(){const name=rn.value.trim(),email=re.value.trim(),password=rp.value,phone=rphone.value.replace(/\D/g,'');if(!name||!email||password.length<8||!/^\d{10}$/.test(phone)){alert('Enter name, email, 8+ character password and valid mobile number.');return}try{await api('/api/auth/request-msg91-registration',{method:'POST',body:{phone}});const flow=++__msg91FlowId;const accessToken=await openMsg91Verification(phone);if(flow!==__msg91FlowId)return;const d=await api('/api/auth/register-msg91',{method:'POST',body:{name,email,password,phone,accessToken}});session(d);closeM();toast('✓ Account created')}catch(e){showRegisterPanel();alert(e.message||'Mobile verification failed')}}
...
document.querySelector('.nav')?.addEventListener('click',e=>{const el=e.target.closest('[data-category]');if(el){e.preventDefault();e.stopPropagation();cat(el.dataset.category)}});
 ...
 restoreSession().finally(()=>{save();loadSiteLogo();load();loadSlides();loadShopCategories();updateHelpUnreadBadge();if(window.__helpUnreadTimer)clearInterval(window.__helpUnreadTimer);window.__helpUnreadTimer=setInterval(updateHelpUnreadBadge,2500);setTimeout(showOfferPopup,1200);setTimeout(()=>warmMsg91().catch(()=>{}),400);});
});
