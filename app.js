...
function makeAdminDashboardDropdowns(){
 const headings=[...document.querySelectorAll('.modal h3')];
 const wanted=['🚚 Orders','💬 Ashwini Help Desk Notifications','📞 Customer Help & Callback Requests','❓ Customer Questions'];
 headings.forEach(heading=>{
  if(!wanted.some(label=>heading.textContent.trim().startsWith(label)))return;
  const content=heading.nextElementSibling;
  if(!content||!content.classList.contains('admin-table-wrap'))return;
  const dropdown=document.createElement('details');dropdown.className='admin-dashboard-dropdown';
  const summary=document.createElement('summary');summary.className='inventory-launch';
  while(heading.firstChild)summary.appendChild(heading.firstChild);
  const arrow=document.createElement('span');arrow.className='admin-dropdown-arrow';arrow.setAttribute('aria-hidden','true');arrow.textContent='⌄';summary.appendChild(arrow);
  const body=document.createElement('div');body.className='admin-dashboard-dropdown-content';
  heading.replaceWith(dropdown);dropdown.append(summary,body);body.appendChild(content);
 });
}
