...
function openM(html){const m=document.getElementById('modal'),b=document.getElementById('body');if(!m||!b)return;if(m.style.display==='flex'&&b.innerHTML)modalBackStack.push(b.innerHTML);b.innerHTML=`<button class="modal-back" type="button" onclick="modalBack()">← Back</button>${html}`;enhancePasswordInputs(b);m.style.zIndex='';m.style.display='flex';m.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';if(String(html).includes('Ashwini Admin Dashboard'))requestAnimationFrame(addAppearanceDashboardBox);document.getElementById('modalClose')?.focus()}
...
function closeM(){if(window.__helpChatTimer){clearInterval(window.__helpChatTimer);window.__helpChatTimer=null}if(window.__helpChatStream){window.__helpChatStream.close();window.__helpChatStream=null}modalBackStack=[];const m=document.getElementById('modal');if(!m)return;m.style.display='none';m.style.zIndex='';m.setAttribute('aria-hidden','true');document.body.style.overflow=''}
...
async function detail(id){
 ...
 openM(`<div class="detail">
  ...
  <section class="mobile-product-suggestions"><h3>Related products</h3><div class="suggestion-grid">${related.map(x=>`<button type="button" class="suggestion-card" onclick="detail(${x.id})"><img src="${esc(x.image||'')}" alt="${esc(x.name)}"><span>${esc(x.name)}</span><b>₹${Number(x.price||0).toLocaleString('en-IN')}</b></button>`).join('')||'<p>More matching products will appear here.</p>'}</div><h3>Recommended for you</h3><div class="suggestion-grid">${recommended.map(x=>`<button type="button" class="suggestion-card" onclick="detail(${x.id})"><img src="${esc(x.image||'')}" alt="${esc(x.name)}"><span>${esc(x.name)}</span><b>₹${Number(x.price||0).toLocaleString('en-IN')}</b></button>`).join('')}</div></section>${policySections()}${securitySection()}${qaHtml}${reviewsHtml}
  </div></div>`);
 bindImageZoom(p.id);
