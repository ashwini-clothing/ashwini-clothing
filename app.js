...
async function orderDetails(id){
 try{
  ...
  openM(`<h2>Order #${o.id}</h2><p><b>Ordered:</b> ${new Date(o.created_at||'').toLocaleString('en-IN')}</p>${o.status==='DELIVERED'&&o.updated_at?`<p><b>Delivered:</b> ${new Date(o.updated_at).toLocaleString('en-IN')}</p>`:''}<p><b>Status:</b> ${esc(label(o.status))}</p><p><b>Payment:</b> ${esc(o.payment_status||'PENDING')} (${esc(o.payment_method||'')})</p><p><b>Delivery address:</b> ${esc(o.address)}</p><h3>Items</h3>${detailItems||'<p>No product items found for this order.</p>'}<div class="total">Total: ₹${Number(o.total).toLocaleString('en-IN')}</div>${returnBlock}<button class="gold" type="button" onclick="track(${o.id})">🚚 Track Order</button>`)
