...
function refreshAccountHeader(){const link=document.getElementById('accountTopLink');if(!link)return;const small=link.querySelector('small'),bold=link.querySelector('b');if(user){const short=String(user.name||'Customer').trim().split(/\s+/)[0]||'Customer',mobileShort=short.slice(0,10);if(small)small.textContent=`Hello, ${short}`;if(bold)bold.textContent='Account & Lists';link.dataset.mobileLabel=`Hello, ${mobileShort}`;link.setAttribute('aria-label',`Account & Lists for ${short}`)}else{if(small)small.textContent='Hello, sign in';if(bold)bold.textContent='Account & Lists';link.dataset.mobileLabel='Sign in';link.setAttribute('aria-label','Sign in to Ashwini')}}
