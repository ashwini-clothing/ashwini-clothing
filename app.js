...
async function accountMenu(){
 ...
 <button class="account-menu-card" type="button" onclick="closeAccountMenu();savedDeliveryAddress()"><b>📍 Saved Delivery Address</b><small>View or update your default order address</small><span class="account-chevron" aria-hidden="true">›</span></button>
...
async function savedDeliveryAddress(){try{const d=await api('/api/me/delivery-address'),a=d.address||{};openM(`<h2>📍 Saved Delivery Address</h2><p>This address is saved automatically after a successful order. You can update it anytime.</p><div class="form"><label><b>Full Name</b></label><input id="savedAddressName" value="${esc(a.name||user?.name||'')}" autocomplete="name"><label><b>Delivery Mobile</b></label><input id="savedAddressPhone" value="${esc(a.phone||user?.phone||'')}" inputmode="numeric" maxlength="10" autocomplete="tel"><label><b>Complete Address</b></label><textarea id="savedAddressLine" rows="4" maxlength="500" placeholder="House / Flat / Street / Area / Landmark">${esc(a.address_line||'')}</textarea><label><b>PIN Code</b></label><input id="savedAddressPin" value="${esc(a.pincode||'')}" inputmode="numeric" maxlength="6"><label><b>City</b></label><input id="savedAddressCity" value="${esc(a.city||'')}"><label><b>State</b></label><input id="savedAddressState" value="${esc(a.state||'')}"><button class="gold" type="button" onclick="saveDeliveryAddress()">Save Address</button><button type="button" onclick="closeM()">Cancel</button></div>`)}catch(e){alert(e.message||'Could not load saved address')}}
async function saveDeliveryAddress(){const body={name:document.getElementById('savedAddressName')?.value,phone:document.getElementById('savedAddressPhone')?.value,address_line:document.getElementById('savedAddressLine')?.value,pincode:document.getElementById('savedAddressPin')?.value,city:document.getElementById('savedAddressCity')?.value,state:document.getElementById('savedAddressState')?.value};try{await api('/api/me/delivery-address',{method:'PUT',body});toast('✓ Delivery address saved');savedDeliveryAddress()}catch(e){alert(e.message||'Address could not be saved')}}
...
async function continueCustomerLogin(){
 ...
 if(!isMobileIdentifier(identifier)){
  try{window.__pendingLoginIdentifier=identifier;const d=await api('/api/auth/request-login-otp',{method:'POST',body:{identifier}});openM(`<div class="amazon-login-wrap"><div class="ashwini-login-logo" aria-label="Ashwini">ASHWINI</div><h2>Enter email OTP</h2><p class="login-account-id">${esc(identifier)} <button type="button" class="text-change" onclick="auth()">Change</button></p><div class="form"><label class="login-label"><b>Enter OTP</b></label><input id="loginOtp" inputmode="numeric" maxlength="6" autocomplete="one-time-code" placeholder="6-digit OTP" autofocus><button class="gold amazon-continue-btn" type="button" onclick="verifyLoginOtpFor()">Sign in</button><small id="loginOtpHint">${d.devOtp?`Demo OTP: ${esc(d.devOtp)}`:'OTP sent to your registered email. It expires in about 5 minutes.'}</small><button type="button" class="linkbtn" onclick="resendLoginOtp()">Resend OTP</button><button type="button" class="linkbtn" onclick="auth()">← Back</button></div></div>`)}catch(e){showLoginNotice(e.message||'Email OTP could not be sent')}finally{if(continueButton){continueButton.disabled=false;continueButton.textContent='Continue'}}return;
 }
 const phone=identifier.replace(/\D/g,''),flowId=++__msg91FlowId;
 try{await api('/api/auth/check-customer-login',{method:'POST',body:{identifier:phone}});if(flowId!==__msg91FlowId)return;if(continueButton)continueButton.textContent='Opening mobile OTP…';const accessToken=await openMsg91Verification(phone);if(flowId!==__msg91FlowId)return;const verified=await api('/api/auth/verify-msg91-login',{method:'POST',body:{identifier:phone,accessToken}});if(flowId!==__msg91FlowId)return;session(verified);closeM();toast('✓ Signed in')}catch(e){if(flowId!==__msg91FlowId)return;auth(phone,e.message||'Mobile verification could not be completed.')}finally{if(continueButton){continueButton.disabled=false;continueButton.textContent='Continue'}}
}
async function openMsg91RegistrationVerification(phone){
 const [cfg]=await warmMsg91();
 return new Promise((resolve,reject)=>{
  let finished=false;const finish=(fn,value)=>{if(finished)return;finished=true;window.__msg91RegistrationVerify=null;fn(value)},verified=data=>{const token=msg91AccessToken(data);token?finish(resolve,token):finish(reject,new Error('MSG91 verified the OTP but did not return a verification token.'))};
  ...
  try{window.initSendOTP(configuration);const providerSend=window.sendOtp;if(typeof providerSend!=='function')throw new Error('MSG91 mobile OTP service is not ready.');providerSend('91'+phone,()=>{openM(`<div class="amazon-login-wrap"><div class="ashwini-login-logo" aria-label="Ashwini">ASHWINI</div><h2>Enter mobile OTP</h2><p class="login-account-id">+91 ${esc(phone)}</p><div class="form"><label class="login-label"><b>6-digit mobile OTP</b></label><input id="registrationMobileOtp" inputmode="numeric" maxlength="6" autocomplete="one-time-code" placeholder="Enter mobile OTP" autofocus><small id="registrationMobileOtpHint">OTP sent securely to your mobile.</small><button type="button" class="gold amazon-continue-btn" onclick="verifyMsg91RegistrationOtp()">Verify mobile</button><button type="button" class="linkbtn" onclick="showRegisterPanel()">← Start again</button></div></div>`);window.__msg91RegistrationVerify={verified,reject}},error=>finish(reject,new Error(error?.message||error?.error||'Mobile OTP could not be sent.')))}catch(error){finish(reject,error)}
 });
}
function verifyMsg91RegistrationOtp(){const otp=(document.getElementById('registrationMobileOtp')?.value||'').trim(),flow=window.__msg91RegistrationVerify;if(!/^\d{4,8}$/.test(otp)){alert('Enter the mobile OTP.');return}if(!flow||typeof window.verifyOtp!=='function'){alert('Mobile verification session expired. Please start again.');showRegisterPanel();return}const button=[...document.querySelectorAll('.amazon-continue-btn')].find(b=>b.offsetParent!==null);if(button){button.disabled=true;button.textContent='Verifying…'}window.verifyOtp(otp,flow.verified,error=>{if(button){button.disabled=false;button.textContent='Verify mobile'};alert(error?.message||error?.error||'Incorrect or expired mobile OTP.')})}
...
function showRegisterPanel(){
 ...
 <button type="button" class="gold" onclick="sendRegistrationOtp()">Verify mobile & create account</button><button type="button" class="linkbtn" data-auth-action="back-signin">← Back to Sign in</button></div></div>`);
}
...
async function register(){return sendRegistrationOtp()}
...
function enhanceCheckoutAddressFields(){
 ...
 const mobile=document.getElementById('mobile');if(mobile){mobile.placeholder='Delivery mobile number (10 digits)';mobile.autocomplete='tel';mobile.required=true;mobile.pattern='[6-9][0-9]{9}';mobile.setAttribute('aria-required','true');mobile.setAttribute('aria-describedby','checkoutMobileWarning');mobile.addEventListener('input',()=>{mobile.value=mobile.value.replace(/\D/g,'').slice(0,10);validateCheckoutMobile(false)});const warning=document.createElement('small');warning.id='checkoutMobileWarning';warning.setAttribute('role','status');warning.style.gridColumn='1/-1';warning.style.marginTop='-6px';mobile.after(warning);validateCheckoutMobile(false)}
 ...
 const error=document.createElement('div');error.id='checkoutAddressError';error.className='checkout-address-error';error.setAttribute('role','alert');error.textContent='Please fill House / Flat No., Village / Area and Landmark.';fragment.appendChild(error);address.before(fragment);syncCheckoutAddress();enhanceSavedAddressOption();
}
async function enhanceSavedAddressOption(){const address=document.getElementById('address'),card=address?.closest('.checkout-card');if(!card||card.dataset.savedAddressChecked==='1')return;card.dataset.savedAddressChecked='1';try{const d=await api('/api/me/delivery-address'),a=d.address;if(!a)return;window.__savedDeliveryAddress=a;const box=document.createElement('div');box.className='coupon-box';box.innerHTML=`<b>📍 Saved delivery address</b><p style="margin:6px 0"><small>${esc(a.name)} · ${esc(a.phone)}<br>${esc(a.address_line)}, ${esc(a.city)}, ${esc(a.state)} — ${esc(a.pincode)}</small></p><button class="wishlist" type="button" onclick="useSavedDeliveryAddress()">Use this address</button> <button type="button" class="linkbtn" onclick="savedDeliveryAddress()">Edit</button>`;card.querySelector('h3')?.after(box)}catch{}}
function useSavedDeliveryAddress(){const a=window.__savedDeliveryAddress;if(!a)return;const set=(id,value)=>{const el=document.getElementById(id);if(el)el.value=value||''};set('fullName',a.name);set('mobile',a.phone);set('pin',a.pincode);const parts=String(a.address_line||'').split(',').map(x=>x.trim()).filter(Boolean);set('checkoutHouse',parts.shift()||a.address_line);set('checkoutLandmark',parts.length>1?parts.pop():'Near saved location');set('checkoutVillage',parts.join(', ')||a.city);syncCheckoutAddress();validateCheckoutMobile(false);addressPinChanged();toast('✓ Saved address selected')}
...
window.confirmRegistrationEmail=confirmRegistrationEmail;window.sendRegistrationOtp=sendRegistrationOtp;window.verifyMsg91RegistrationOtp=verifyMsg91RegistrationOtp;
