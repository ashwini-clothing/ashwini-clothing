...
function deliveryDayDateText(d){const from=new Date(d.from),to=new Date(d.to),fmt={weekday:'short',day:'numeric',month:'short'};return from.toLocaleDateString('en-IN',fmt)+' – '+to.toLocaleDateString('en-IN',fmt)}
...
function privacyControls(){openM(`<h2>🗂️ Your Data & Account</h2><p>You can download a copy of your Ashwini account data or request account deletion.</p><div class="help-list"><button class="help-row account-action" type="button" onclick="downloadPersonalData()"><div><b>⬇️ Download your data</b><small>Profile, orders, returns, reviews and account activity</small></div><span>›</span></button><button class="help-row account-action" type="button" onclick="accountDeletionForm()"><div><b>🗑️ Request account deletion</b><small>Your account will remain active until the request is safely reviewed</small></div><span>›</span></button></div><button type="button" onclick="loginSecurity()">Back</button>`)}
...
async function orderDetails(id){
 try{
  ...
  openM(`<h2>Order #${o.id}</h2><p><b>Ordered:</b> ${new Date(o.created_at||'').toLocaleString('en-IN')}</p>${o.status==='DELIVERED'&&o.updated_at?`<p><b>Delivered:</b> ${new Date(o.updated_at).toLocaleString('en-IN')}</p>`:''}<p><b>Status:</b> ${esc(label(o.status))}</p><p><b>Payment:</b> ${esc(o.payment_status||'PENDING')} (${esc(o.payment_method||'')})</p>${o.refund_status?`<p><b>Refund:</b> ${esc(label(o.refund_status))}</p>`:''}<p><b>Delivery address:</b> ${esc(o.address)}</p><h3>Items</h3>${detailItems||'<p>No product items found for this order.</p>'}<div class="total">Total: ₹${Number(o.total).toLocaleString('en-IN')}</div>${returnBlock}<div style="display:flex;gap:8px;flex-wrap:wrap"><button class="gold" type="button" onclick="track(${o.id})">🚚 Track Order</button>${cancelButton}</div>`)
...
window.activeSessions=activeSessions;window.signOutSession=signOutSession;window.logoutAllDevices=logoutAllDevices;window.privacyControls=privacyControls;window.downloadPersonalData=downloadPersonalData;window.accountDeletionForm=accountDeletionForm;window.submitAccountDeletion=submitAccountDeletion;
