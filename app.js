...
const zoomState={};
function applyZoom(id){const st=zoomState[id]||{scale:1};const img=document.getElementById(`gallery-main-${id}`);const label=document.getElementById(`zoom-level-${id}`);if(img)img.style.transform=`scale(${st.scale})`;if(label)label.textContent=`${Math.round(st.scale*100)}%`}
function zoomImage(id,delta){const st=zoomState[id]||(zoomState[id]={scale:1});st.scale=Math.max(1,Math.min(3,Math.round((st.scale+delta)*10)/10));applyZoom(id)}
function resetZoom(id){zoomState[id]={scale:1};applyZoom(id)}
function bindImageZoom(id){const wrap=document.getElementById(`gallery-main-wrap-${id}`),img=document.getElementById(`gallery-main-${id}`);if(!wrap||!img||wrap.dataset.zoomBound)return;wrap.dataset.zoomBound='1';let startDist=0,startScale=1;wrap.addEventListener('wheel',e=>{e.preventDefault();zoomImage(id,e.deltaY<0?.2:-.2)},{passive:false});wrap.addEventListener('dblclick',e=>{e.preventDefault();const st=zoomState[id]||{scale:1};st.scale=st.scale>1?1:2;applyZoom(id)});wrap.addEventListener('touchstart',e=>{if(e.touches.length===2){startDist=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);startScale=(zoomState[id]||{scale:1}).scale}},{passive:true});wrap.addEventListener('touchmove',e=>{if(e.touches.length===2){e.preventDefault();const d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);const st=zoomState[id]||(zoomState[id]={scale:1});st.scale=Math.max(1,Math.min(3,Math.round((startScale*(d/startDist))*10)/10));applyZoom(id)}},{passive:false})}
...
async function qaSection(id){
 try{
  ...
  const cards=q.length?q.map(x=>`<div class="review qa-item"><b>Q:</b> ${esc(x.question)} <small>— ${esc(x.asker_name||'Customer')}</small>${x.answers?.length?x.answers.map(a=>`<div class="qa-answer"><b>A (${esc(a.role==='admin'?'Admin':'Customer')}):</b> ${esc(a.answer)} <small>— ${esc(a.name||'Customer')}</small></div>`).join(''):'<div class="qa-empty">No answer yet.</div>'}<div class="form" style="margin-top:8px"><input id="answer-${x.id}" placeholder="Write an answer"><button class="gold" type="button" onclick="answerQuestion(${x.id},${id})">Answer</button></div></div>`).join(''):'<p>No questions yet. Be the first to ask.</p>';
...
async function manageProfile(){try{const me=await api('/api/me');openM(`<h2>👤 Manage Profile</h2><div class="form"><label>Name</label><input id="profileName" value="${esc(me.name||'')}"><label>Email</label><input id="profileEmail" type="email" value="${esc(me.email||'')}"><label>Mobile Number</label><input id="profilePhone" inputmode="numeric" maxlength="10" value="${esc(me.phone||'')}"><button class="gold" type="button" onclick="saveProfile()">Save Changes</button></div>`)}catch(e){alert(e.message)}}
async function saveProfile(){try{const d=await api('/api/me',{method:'PATCH',body:{name:document.getElementById('profileName')?.value,email:document.getElementById('profileEmail')?.value,phone:document.getElementById('profilePhone')?.value}});session(d);toast('✓ Profile updated');accountMenu()}catch(e){alert(e.message)}}
async function returnsPanel(){try{const [os,rs,evs]=await Promise.all([api('/api/orders'),api('/api/returns'),api('/api/return-events')]);const rmap=new Map();rs.forEach(r=>{const oid=Number(r.order_id),current=rmap.get(oid);if(!current||Number(r.id)>Number(current.id))rmap.set(oid,r)});const byReturn=new Map();evs.forEach(e=>{const a=byReturn.get(Number(e.return_id))||[];a.push(e);byReturn.set(Number(e.return_id),a)});const delivered=os.filter(o=>o.status==='DELIVERED');const label=v=>String(v||'').replaceAll('_',' ');const canCancel=r=>['REQUESTED','APPROVED','PICKUP_SCHEDULED'].includes(String(r.status));const canRequestAgain=r=>!r||['REJECTED','CANCELLED'].includes(String(r.status));const rows=delivered.map(o=>{const r=rmap.get(Number(o.id));const formId=`return-form-${o.id}`;const history=(r?(byReturn.get(Number(r.id))||[]).slice(0,12):[]);return `<div class="cartrow" style="grid-template-columns:1fr auto"><div><b>Order #${o.id}</b><br>₹${Number(o.total).toLocaleString('en-IN')}<br><small>${r?`Return #${r.id}: ${esc(label(r.status))} · ${esc(r.reason)} · ${esc(r.request_type||'REPLACEMENT')}${r.pickup_at?` · Pickup: ${esc(r.pickup_at)}`:''}`:'Eligible for return request'}</small>${r?.admin_note?`<br><small>Admin note: ${esc(r.admin_note)}</small>`:''}${history.length?`<div style="margin-top:8px;padding-top:8px;border-top:1px solid #eee"><b>Return updates</b>${history.map(e=>`<div style="margin-top:5px"><small><b>${esc(e.title)}</b> · ${esc(e.created_at)}</small><br><small>${esc(e.message)}</small></div>`).join('')}</div>`:''}</div><div>${r?`<span class="status-pill">${esc(label(r.status))}</span>${canCancel(r)?`<br><button type="button" style="margin-top:8px" onclick="cancelReturn(${r.id})">Cancel Return Request</button>`:''}${canRequestAgain(r)?`<br><button class="gold" type="button" style="margin-top:8px" onclick="toggleReturnForm(${o.id})">Request Return / Replace Again</button>`:''}`:`<button class="gold" type="button" onclick="toggleReturnForm(${o.id})">Request Return / Replace</button>`}</div></div>${canRequestAgain(r)?`<div id="${formId}" style="display:none;margin:-6px 0 14px;padding:14px;background:#fffaf7;border:1px solid #eaded8;border-radius:9px"><div class="form"><label><b>Return reason</b></label><select id="return-reason-${o.id}" onchange="syncReturnOption(${o.id})"><option value="">Select reason</option><option>Size too small</option><option>Size too large</option><option>Colour / Style issue</option><option>Damaged / Defective</option><option>Other</option></select><label><b>What do you want?</b></label><select id="return-type-${o.id}" onchange="syncReturnOption(${o.id})"><option value="EXCHANGE">Exchange / Replace with another size or colour</option><option value="REPLACEMENT">Replacement</option></select><div id="return-extra-${o.id}" style="display:grid;grid-template-columns:1fr 1fr;gap:8px"><input id="return-size-${o.id}" placeholder="New size (optional)"><input id="return-color-${o.id}" placeholder="New colour (optional)"></div><button class="gold" type="button" onclick="submitReturn(${o.id})">Submit Return Request</button><button type="button" onclick="toggleReturnForm(${o.id})">Cancel</button></div></div>`:''}`}).join('');openM(`<h2>↩️ Returns & Replacements</h2><p>Request returns on this same page. Return status, pickup and inspection updates are shown here.</p>${rows||'<p>No delivered orders are currently eligible for a return request.</p>'}`)}catch(e){alert(e.message)}}
...
async function confirmCancelReturn(id){try{await api(`/api/returns/${id}/cancel`,{method:'PATCH'});toast('✓ Return request cancelled');returnsPanel()}catch(e){alert(e.message)}}
...
async function customerHelp(){
 try{
  ...
  <div class="help-list">
   <button class="help-row" type="button" onclick="orders()"><div><b>📦 Your Orders</b><small>View, track and manage your orders</small></div><span>›</span></button>
   <button class="help-row" type="button" onclick="returnsPanel()"><div><b>↩️ Returns & Replacements</b><small>Request a return and check return status</small></div><span>›</span></button>
...
function auth(){
 ...
 openM(`<h2>Sign in to Ashwini</h2>
 <div class="form">
 <div class="login-tabs"><button id="passwordLoginTab" class="gold" type="button" onclick="showLoginMode('password')">Password Login</button><button id="otpLoginTab" type="button" onclick="showLoginMode('otp')">OTP Login</button></div>
 <div id="passwordLoginBox"><input id="email" placeholder="Registered Email"><input id="pass" type="password" placeholder="Password"><button class="gold" onclick="login()">Sign in</button><div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:8px"><button type="button" class="linkbtn" onclick="forgotPasswordFlow()">Forgot Password?</button><button type="button" class="linkbtn" onclick="forgotLoginIdFlow()">Forgot Login ID?</button></div></div>
 <div id="otpLoginBox" style="display:none"><input id="loginIdentifier" placeholder="Email or 10-digit mobile number"><button type="button" onclick="sendLoginOtp()">Send OTP</button><input id="loginOtp" inputmode="numeric" maxlength="6" placeholder="Enter 6-digit OTP"><button class="gold" onclick="verifyLoginOtp()">Verify OTP & Sign in</button><small id="loginOtpHint">OTP will be shown in demo mode. Connect a real SMS/email provider before launch.</small></div>
 <hr><h3>New Buyer — Mobile OTP</h3><input id="rn" placeholder="Full name"><input id="re" placeholder="Email"><input id="rp" type="password" placeholder="Password (8+ characters)"><input id="rphone" inputmode="numeric" maxlength="10" placeholder="10-digit mobile number"><button type="button" onclick="sendOtp()">Send OTP</button><input id="rotp" inputmode="numeric" maxlength="6" placeholder="Enter 6-digit OTP"><button class="gold" onclick="register()">Verify OTP & Create Account</button><small id="otpHint">For demo mode, the OTP will be shown after Send OTP.</small>
 <hr><details><summary>First-time store admin setup</summary><p class="admin-note">Use this only once on your own store before giving customers access.</p><input id="an" placeholder="Store owner name"><input id="ae" placeholder="Admin email"><input id="apw" type="password" placeholder="Admin password (8+ characters)"><button class="gold" onclick="setupAdmin()">Create Store Admin</button></details></div>`);
}
function showLoginMode(mode){const p=document.getElementById('passwordLoginBox'),o=document.getElementById('otpLoginBox'),pt=document.getElementById('passwordLoginTab'),ot=document.getElementById('otpLoginTab');if(p)p.style.display=mode==='password'?'block':'none';if(o)o.style.display=mode==='otp'?'block':'none';pt?.classList.toggle('gold',mode==='password');ot?.classList.toggle('gold',mode==='otp')}
async function sendLoginOtp(){const el=document.getElementById('loginIdentifier'),hint=document.getElementById('loginOtpHint'),identifier=(el?.value||'').trim();if(!identifier){hint.textContent='Enter your registered email or mobile number.';return}try{const d=await api('/api/auth/request-login-otp',{method:'POST',body:{identifier}});hint.textContent=`Demo OTP: ${d.devOtp} · ${d.channel==='mobile'?'Mobile':'Email'} OTP generated.`;toast('✓ OTP generated')}catch(e){hint.textContent=e.message;alert(e.message)}}
async function verifyLoginOtp(){const identifier=(document.getElementById('loginIdentifier')?.value||'').trim(),otp=(document.getElementById('loginOtp')?.value||'').trim();try{const d=await api('/api/auth/verify-login-otp',{method:'POST',body:{identifier,otp}});session(d);closeM()}catch(e){alert(e.message)}}
...
function logout(){
 localStorage.removeItem('ashwiniToken');
 localStorage.removeItem('ashwiniUser');
 token='';
 user=null;
 const a=document.getElementById('admin');
 if(a)a.style.display='none';
 auth();
 toast('✓ Logged out. Sign in again to continue.');
}
async function orders(){try{const [os,evs]=await Promise.all([api('/api/orders'),api('/api/return-events')]);const label=v=>String(v||'').replaceAll('_',' ');const byReturn=new Map();evs.forEach(e=>{const a=byReturn.get(Number(e.return_id))||[];a.push(e);byReturn.set(Number(e.return_id),a)});const rows=os.map(x=>{const r=x.return_request;const events=r?(byReturn.get(Number(r.id))||[]).slice(0,5):[];return `<div class="cartrow" style="grid-template-columns:1fr auto"><div><b>Order #${x.id}</b><br>${esc((x.items||[]).map(i=>`${i.name} × ${i.quantity}`).join(', '))}<br><span>₹${Number(x.total).toLocaleString('en-IN')}</span><br><small>Status: ${esc(label(x.status))} · Payment: ${esc(x.status==='DELIVERED'?'PAID':(x.payment_status||'PENDING'))}</small>${r?`<div style="margin-top:8px;padding-top:8px;border-top:1px solid #eee"><small><b>↩️ Return & Replacement: ${esc(label(r.status))}</b>${r.pickup_at?` · Pickup: ${esc(r.pickup_at)}`:''}</small>${events.map(e=>`<div style="margin-top:4px"><small>• ${esc(e.title)} — ${esc(e.message)}</small></div>`).join('')}</div>`:''}</div><div><button class="gold" type="button" onclick="orderDetails(${x.id})">View Details</button>${r?`<br><button type="button" style="margin-top:7px" onclick="returnsPanel()">View Return Status</button>`:''}</div></div>`}).join('');openM(`<h2>📦 My Orders</h2><p>Every order shows its current delivery, payment and return/replacement status.</p>${rows||'<p>No orders yet. Place your first order and it will appear here.</p>'}`)}catch(e){alert('My Orders could not load: '+e.message)}}
async function orderDetails(id){try{const [o,evs]=await Promise.all([api('/api/orders/'+id),api('/api/return-events')]);const label=v=>String(v||'').replaceAll('_',' ');const r=o.return_request;const events=r?(evs.filter(e=>Number(e.return_id)===Number(r.id)).slice(0,12)):[];const canCancel=r&&['REQUESTED','APPROVED','PICKUP_SCHEDULED'].includes(String(r.status));const canRequestAgain=r&&['REJECTED','CANCELLED'].includes(String(r.status));openM(`<h2>Order #${o.id}</h2><p><b>Status:</b> ${esc(label(o.status))}</p><p><b>Payment:</b> ${esc(o.status==='DELIVERED'?'PAID':(o.payment_status||'PENDING'))} (${esc(o.payment_method||'')})</p><p><b>Delivery address:</b> ${esc(o.address)}</p><h3>Items</h3>${(o.items||[]).map(i=>`<div class="cartrow"><div><b>${esc(i.name)}</b><br>Size ${esc(i.size)} · Qty ${i.quantity}</div><b>₹${Number(i.unit_price*i.quantity).toLocaleString('en-IN')}</b></div>`).join('')}<div class="total">Total: ₹${Number(o.total).toLocaleString('en-IN')}</div>${r?`<div class="wishlist-section"><h3>↩️ Return & Replacement</h3><p><b>Return #${r.id}</b> · <span class="status-pill">${esc(label(r.status))}</span></p><p>Reason: ${esc(r.reason)} · Option: ${esc(r.request_type||'REPLACEMENT')}${r.pickup_at?`<br>Pickup: ${esc(r.pickup_at)}`:''}</p>${r.admin_note?`<p>Admin note: ${esc(r.admin_note)}</p>`:''}${events.length?`<div style="margin-top:10px;padding-top:10px;border-top:1px solid #eee"><b>Return notifications</b>${events.map(e=>`<div style="margin-top:6px"><small><b>${esc(e.title)}</b> · ${esc(e.created_at)}</small><br><small>${esc(e.message)}</small></div>`).join('')}</div>`:''}<div style="margin-top:10px"><button type="button" onclick="returnsPanel()">View Return Updates</button>${canCancel?`<button type="button" style="margin-left:8px" onclick="cancelReturn(${r.id})">Cancel Return Request</button>`:''}${canRequestAgain?`<button class="gold" type="button" style="margin-left:8px" onclick="returnsPanel()">Request Return / Replace Again</button>`:''}</div></div>`:''}<button class="gold" type="button" onclick="track(${o.id})">🚚 Track Order</button>`)}catch(e){alert('Order details could not load: '+e.message)}}
...
async function dashboard(){
 if(user?.role!=='admin')return alert('Admin only');
 const [s,o,p,q,h]=await Promise.all([api('/api/admin/stats'),api('/api/admin/orders'),api('/api/products'),api('/api/admin/questions'),api('/api/admin/customer-help')]);
...
async function adminStoreProfile(){
 ...
 try{
  const x=await api('/api/store-profile');
  openM(`<h2>🏢 Edit Ashwini Information</h2><div class="admin-note">Edit the customer-facing Ashwini information and logo. Existing website functions are not changed.</div><div class="admin-form"><label class="full">Store Logo<input id="sp_logo" type="file" accept="image/png,image/jpeg,image/webp,image/gif" onchange="previewStoreLogo(this)"><small>Upload a new logo. The current logo remains unchanged until you save.</small><div id="sp_logo_preview" style="margin-top:10px"><img src="${esc(x.logo_data||'logo-clear.png')}" alt="Current logo" style="max-width:180px;max-height:100px;object-fit:contain;border:1px solid #ddd;border-radius:8px;padding:8px;background:#fff"></div></label><label>Page Heading<input id="sp_title" value="${esc(x.about_title||'About Ashwini Clothing')}"></label><label>Email<input id="sp_email" value="${esc(x.email||'ashwiniweb88@gmail.com')}"></label><label class="full">History / About Ashwini<textarea id="sp_history" rows="9" placeholder="Write Ashwini Clothing history, story, values, journey etc.">${esc(x.history||'')}</textarea></label><label>Address<input id="sp_address" value="${esc(x.address||'')}"></label><label>City<input id="sp_city" value="${esc(x.city||'')}"></label><label>State<input id="sp_state" value="${esc(x.state||'')}"></label><label>PIN Code<input id="sp_pin" value="${esc(x.pincode||'')}" maxlength="6"></label><label>Phone<input id="sp_phone" value="${esc(x.phone||'')}"></label></div><div class="admin-actions" style="margin-top:14px"><button class="gold" type="button" onclick="saveStoreProfile()">💾 Save Information</button><button type="button" onclick="dashboard()">Back</button></div>`);
 }catch(e){alert(e.message)}
}
async function saveStoreProfile(){
 try{
  let logo_data='';const f=document.getElementById('sp_logo')?.files?.[0];if(f){if(f.size>10*1024*1024)throw Error('Logo image must be 10MB or smaller');logo_data=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result||''));r.onerror=()=>reject(Error('Could not read logo file'));r.readAsDataURL(f)});}const body={about_title:sp_title.value.trim(),history:sp_history.value,address:sp_address.value.trim(),city:sp_city.value.trim(),state:sp_state.value.trim(),pincode:sp_pin.value.trim(),email:sp_email.value.trim(),phone:sp_phone.value.trim(),logo_data};
  await api('/api/admin/store-profile',{method:'PATCH',body});toast('✓ Ashwini information saved');await loadSiteLogo();storeProfilePage();
...
async function adminAnswerQuestion(questionId,productId){
 ...
 const text=String(answer).trim();if(!text){toast('Please write an answer');return}
 try{await api(`/api/questions/${questionId}/answers`,{method:'POST',body:{answer:text}});toast('Answer added');dashboard()}catch(e){alert(e.message||'Could not add answer')}
}
...
async function showOfferDetails(offerId){
 try{
  ...
  openM(`<div style="text-align:center">${img}<h2>🎁 ${esc(o.title)}</h2><p>${esc(o.description||'')}</p>${Number(o.discount_percent||0)>0?`<div style="font-size:25px;font-weight:800;margin:12px 0">${Number(o.discount_percent)}% OFF</div>`:''}${o.coupon_code?`<p><b>Coupon: ${esc(o.coupon_code)}</b></p>`:''}<button class="gold" type="button" onclick="closeM();setTimeout(()=>{${action}},50)">${esc(o.button_text||'Shop Now')}</button></div>`);
 ...
 try{
  ...
  track.innerHTML=slides.map((x,i)=>{
   ...
   return `<div class="ad-slide ${i===0?'active':''}" data-slide-id="${Number(x.id)||0}">
    <img src="${src}" loading="eager" decoding="async" alt="${esc(x.title||'Ashwini offer')}" onerror="this.style.visibility='hidden'">
    ${(x.title||x.offer_text)?`<div style="position:absolute;left:5%;top:12%;z-index:2;color:#5a2e40;text-shadow:0 1px 2px rgba(255,255,255,.7)"><div style="font-size:28px;font-weight:800">${esc(x.title||'')}</div>${x.offer_text?`<div style="font-size:22px;font-weight:700;margin-top:6px">${esc(x.offer_text)}</div>`:''}</div>`:''}
    ${x.button_text?`<button class="ad-cta" style="position:absolute;left:5%;bottom:10%;z-index:3" type="button" onclick="${esc(x.button_action||'')}">${esc(x.button_text)}</button>`:''}
   </div>`
  }).join('');
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 save();loadSiteLogo();load();loadSlides();loadShopCategories();setTimeout(showOfferPopup,1200);
});
