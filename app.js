...
let category='All', sizes={}, adIndex=0, adTimer, adPaused=false, checkoutItems=null, quickFilters=new Set(), modalBackStack=[];
...
async function lookupDeliveryPin(pin){
 ...
 const mobileResult=document.getElementById('mobileDeliveryResult');
 if(x) x.textContent='Checking PIN location...';
 if(mobileResult)mobileResult.textContent='Checking delivery availability...';
 try{
  const [d,estimate]=await Promise.all([api('/api/pincode/'+encodeURIComponent(pin)),api('/api/delivery-estimate/'+encodeURIComponent(pin))]);
  ...
  if(x)x.innerHTML=`<b>${esc(d.area)}</b>, ${esc(d.city||d.district)}, ${esc(d.state)}<br><small>PIN ${esc(d.pin)}${estimate.deliverable===false?' · Delivery unavailable':' · Expected '+esc(deliveryDateText(estimate))}</small>`;
  if(mobileResult)mobileResult.innerHTML=estimate.deliverable===false?`<b class="delivery-error">${esc(estimate.message||'Delivery is not available for your area.')}</b>`:`<b>✓ ${esc(d.city||d.district)}</b> · Delivery by ${esc(deliveryDateText(estimate))}`;
 }catch(e){if(x)x.textContent=e.message||'PIN code location not found.';if(mobileResult)mobileResult.textContent=e.message||'PIN code could not be checked.'}
}
function checkMobileDeliveryPin(){const pin=(document.getElementById('mobileDeliveryPin')?.value||'').trim();if(!/^\d{6}$/.test(pin)){const x=document.getElementById('mobileDeliveryResult');if(x)x.textContent='Enter a valid 6-digit PIN code.';return}lookupDeliveryPin(pin)}
...
function openM(html){const m=document.getElementById('modal'),b=document.getElementById('body');if(!m||!b)return;const isNewModal=m.style.display!=='flex';if(m.style.display==='flex'&&b.innerHTML)modalBackStack.push(b.innerHTML);b.innerHTML=`<button class="modal-back" type="button" onclick="modalBack()">← Back</button>${html}`;enhancePasswordInputs(b);m.style.zIndex='';m.style.display='flex';m.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';if(isNewModal&&!modalHistoryPushed&&history?.pushState){history.pushState({...history.state,ashwiniModal:true},'',location.href);modalHistoryPushed=true}if(String(html).includes('Ashwini Admin Dashboard'))requestAnimationFrame(addAppearanceDashboardBox);document.getElementById('modalClose')?.focus()}
function modalBack(){const b=document.getElementById('body');if(!b)return;if(!modalBackStack.length){closeM();return}b.innerHTML=modalBackStack.pop();enhancePasswordInputs(b);if(b.innerHTML.includes('Ashwini Admin Dashboard'))requestAnimationFrame(addAppearanceDashboardBox)}
function closeM(fromBrowser=false){if(window.__helpChatTimer){clearInterval(window.__helpChatTimer);window.__helpChatTimer=null}if(window.__helpChatStream){window.__helpChatStream.close();window.__helpChatStream=null}modalBackStack=[];const m=document.getElementById('modal');if(!m)return;m.style.display='none';m.style.zIndex='';m.setAttribute('aria-hidden','true');document.body.style.overflow='';if(modalHistoryPushed){modalHistoryPushed=false;if(!fromBrowser&&history?.state?.ashwiniModal)history.back()}}
window.addEventListener('popstate',()=>{const m=document.getElementById('modal');if(m?.style.display==='flex'){if(modalBackStack.length)modalBack();else closeM(true)}modalHistoryPushed=false});
...
function galleryStep(id,delta){const wrap=document.getElementById(`gallery-main-wrap-${id}`)?.closest('.gallery');const thumbs=[...(wrap?.querySelectorAll('.gallery-thumb')||[])];if(!thumbs.length)return;let current=Math.max(0,thumbs.findIndex(x=>x.classList.contains('active')));thumbs[(current+delta+thumbs.length)%thumbs.length]?.click()}
function recentProductKey(){return user?`ashwiniRecentProducts_${user.id}`:'ashwiniRecentProducts_guest'}
function recentlyViewed(){try{return JSON.parse(localStorage.getItem(recentProductKey())||'[]').map(Number)}catch{return[]}}
function rememberProduct(id){const items=recentlyViewed().filter(x=>x!==Number(id));items.unshift(Number(id));localStorage.setItem(recentProductKey(),JSON.stringify(items.slice(0,12)))}
async function showProductDelivery(id,pin){const out=document.getElementById(`product-auto-delivery-${id}`);if(!out)return;out.textContent='Checking delivery to your saved PIN...';try{const d=await api('/api/delivery-estimate/'+encodeURIComponent(pin));out.innerHTML=d.deliverable===false?`<b class="delivery-error">${esc(d.message||'Delivery is not available for your area.')}</b>`:`<b>📍 Delivery to your saved PIN:</b> ${esc(deliveryDateText(d))} · ${Number(d.minDays)}–${Number(d.maxDays)} days`}catch{out.textContent='Delivery estimate will be shown after PIN check.'}}
function startVoiceSearch(){const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!Recognition){toast('Voice search is not supported in this browser.');return}const r=new Recognition();r.lang='en-IN';r.interimResults=false;r.onresult=e=>{const q=document.getElementById('q');if(q){q.value=e.results[0][0].transcript;load();toast('Searching for: '+q.value)}};r.onerror=()=>toast('Voice search could not start. Please allow microphone access.');r.start()}
function searchByPhoto(input){const file=input?.files?.[0];if(!file)return;const name=file.name.replace(/\.[^/.]+$/,'').replace(/[_-]+/g,' ').trim();const q=document.getElementById('q');if(q){q.value=name;load();toast('Searching using photo name: '+name)}input.value=''}
const zoomState={};
...
async function detail(id){
 ...
 const savedPin=localStorage.getItem('ashwiniDeliveryPin')||'';
 rememberProduct(p.id);
 const recent=recentlyViewed();
 const related=ps.filter(x=>x.id!==p.id&&(x.category===p.category||x.color===p.color)).slice(0,4);
 const recommended=ps.filter(x=>x.id!==p.id&&!related.some(r=>r.id===x.id)).sort((a,b)=>(recent.includes(b.id)?1:0)-(recent.includes(a.id)?1:0)).slice(0,4);
 ...
 openM(`<div class="detail">
  ...
  <div class="delivery-box"><b>📍 Check delivery</b>${user&&savedPin?`<div id="product-auto-delivery-${p.id}" class="delivery-result">Checking your saved delivery PIN...</div>`:''}<div class="pin-row"><input id="pin-${p.id}" maxlength="6" inputmode="numeric" value="${esc(savedPin)}" placeholder="Enter PIN code"><button class="wishlist" type="button" onclick="stop(event);checkDelivery(${p.id})">Check</button></div><div id="delivery-${p.id}" class="delivery-result">Free delivery available after PIN check.</div></div>
  ...
  ${policySections()}${securitySection()}${qaHtml}${reviewsHtml}<section class="mobile-product-suggestions"><h3>Related products</h3><div class="suggestion-grid suggestion-track">${related.map(x=>`<button type="button" class="suggestion-card" onclick="detail(${x.id})"><img src="${esc(x.image||'')}" alt="${esc(x.name)}"><span>${esc(x.name)}</span><b>₹${Number(x.price||0).toLocaleString('en-IN')}</b></button>`).join('')||'<p>More matching products will appear here.</p>'}</div><h3>Recommended for you</h3><div class="suggestion-grid suggestion-track">${recommended.map(x=>`<button type="button" class="suggestion-card" onclick="detail(${x.id})"><img src="${esc(x.image||'')}" alt="${esc(x.name)}"><span>${esc(x.name)}</span><b>₹${Number(x.price||0).toLocaleString('en-IN')}</b></button>`).join('')}</div></section>
  </div></div>`);
 bindImageZoom(p.id);
 if(user&&savedPin)showProductDelivery(p.id,savedPin);
}
...
async function pay(){try{const items=activeCheckoutItems();const full=(document.getElementById('fullName').value||user.name||'').trim(),mobile=document.getElementById('mobile').value.trim(),address=document.getElementById('address').value.trim(),city=document.getElementById('city').value.trim(),state=document.getElementById('state').value.trim(),pin=document.getElementById('pin').value.trim();if(!mobile||!address||!city||!state||!/^\d{6}$/.test(pin))throw Error('Please fill all delivery details with a valid 6-digit PIN');let d=await api('/api/checkout/create',{method:'POST',body:{items,mobile,address:[full,mobile,address,city,state,pin].filter(Boolean).join(', '),payment_method:document.getElementById('payment').value,delivery_state:document.getElementById('state')?.value||'',coupon:(document.getElementById('coupon')?.value||'').trim().toUpperCase()}});if(d.mode==='COD'){if(!d.orderId)throw Error('Order was not created correctly.');if(!checkoutItems)cart=[];save();checkoutItems=null;await orderDetails(d.orderId);toast(`✓ Order #${d.orderId} placed successfully`);return}if(!window.Razorpay)throw Error('Payment gateway is not loaded. COD is available.');let paymentCompleted=false;const r=new Razorpay({key:d.keyId,amount:d.total*100,currency:'INR',name:'Ashwini Clothing',description:'Secure Ashwini payment',order_id:d.razorpayOrderId,prefill:{name:user.name,email:user.email,contact:mobile},config:{display:{sequence:['upi','card','netbanking','wallet'],preferences:{show_default_blocks:true}}},modal:{ondismiss:()=>{if(!paymentCompleted)toast('Order not placed because the payment transaction was not completed.')}},handler:async res=>{try{const verified=await api('/api/checkout/verify',{method:'POST',body:res});paymentCompleted=true;if(!verified.orderId)throw Error('Payment was verified but the order could not be created.');if(!checkoutItems)cart=[];save();checkoutItems=null;showConfirmation(verified.orderId,'PAID')}catch(e){alert(e.message||'Order not placed because the payment transaction was not completed.')}}});r.on('payment.failed',()=>{toast('Order not placed because the payment transaction was not completed.')});r.open()}catch(e){alert(e.message)}}
...
async function buyAgain(){
 try{const os=await api('/api/orders');const seen=new Set(),items=[];for(const o of os){for(const i of (o.items||[])){const k=String(i.product_id)+'|'+String(i.size);if(seen.has(k))continue;seen.add(k);items.push(i)}}if(!items.length){openM('<h2>🔄 Buy Again</h2><p>You have no previous purchased products yet.</p>');return}const ps=await api('/api/products');const cards=items.map(i=>{const p=ps.find(x=>Number(x.id)===Number(i.product_id));if(!p)return '';const size=String(i.size||'M');return `<article class="buy-again-card"><button class="mini" type="button" aria-label="View ${esc(p.name)}" onclick="detail(${p.id})">${p.image?`<img src="${esc(p.image)}" alt="${esc(p.name)}">`:esc(p.emoji||'👕')}</button><div><button class="order-product-title" type="button" onclick="detail(${p.id})">${esc(p.name)}</button><p>Previous size: <b>${esc(size)}</b></p><b>₹${Number(p.price).toLocaleString('en-IN')}</b></div><button class="gold" type="button" onclick="buyAgainOne(${p.id},'${size.replace(/'/g,"\\'")}')">Add Again</button></article>`}).join('');openM(`<h2>🔄 Buy Again</h2>${cards}`)}catch(e){alert(e.message||'Could not load Buy Again')}
}
...
function toggleAdminDashboardSection(name){
 const target=document.querySelector(`.modal .dashboard-section-content[data-dashboard-section-content="${name}"]`);
 if(!target)return;
 const opening=target.hidden;
 document.querySelectorAll('.modal .dashboard-section-content').forEach(x=>x.hidden=true);
...
function upgradeAdminDashboardSections(){
 ...
 const sections=[
  ...
  {name:'callback',label:'📞 Customer Help & Callback Requests',match:t=>t==='📞 Customer Help & Callback Requests'},
  {name:'questions',label:'❓ Customer Questions',match:t=>t.startsWith('❓ Customer Questions')}
 ];
 for(const section of sections){
  const heading=[...document.querySelectorAll('.modal h3')].find(x=>section.match(x.textContent.trim()));
  const content=heading?.nextElementSibling;
  if(!heading||!content||content.classList.contains('dashboard-section-content'))continue;
...
async function adminReturns(){
 ...
 try{
  ...
  const rows=rs.map(r=>{const paidRazorpay=String(r.payment_method||'').toUpperCase()==='RAZORPAY'&&String(r.payment_status||'').toUpperCase()==='PAID'&&r.razorpay_payment_id;const refundable=paidRazorpay&&['RECEIVED','INSPECTION_PASSED','COMPLETED'].includes(String(r.status||''))&&!['PROCESSING','PROCESSED'].includes(String(r.refund_status||''));const refundInfo=r.refund_status?`<br><small><b>Refund: ${esc(String(r.refund_status).replaceAll('_',' '))}</b>${Number(r.refund_amount)>0?` · ₹${Number(r.refund_amount).toLocaleString('en-IN')}`:''}${r.refund_id?`<br>Razorpay: ${esc(r.refund_id)}`:''}</small>`:'';const refundAction=refundable?`<hr style="margin:10px 0;border:0;border-top:1px solid #eaded8"><input id="return-refund-${r.id}" type="number" min="1" max="${Number(r.total||0)}" step="1" value="${Number(r.total||0)}" aria-label="Refund amount" style="width:108px;padding:7px;border:1px solid #d8c7cc;border-radius:7px"> <button class="admin-danger" type="button" onclick="refundReturn(${r.id})">Refund via Razorpay</button><br><small>Only after item received + inspection passed.</small>`:refundInfo||(!paidRazorpay?`<br><small>Razorpay refund not available for ${esc(r.payment_method||'this')} payment.</small>`:`<br><small>Refund unlocks after Received / Inspection Passed.</small>`);return `<tr><td>#${r.id}<br><small>Order #${r.order_id}</small></td><td><b>${esc(r.customer_name||'Customer')}</b><br><small>${esc(r.customer_email||'')}<br>${r.customer_phone?`<a href="tel:${esc(r.customer_phone)}">${esc(r.customer_phone)}</a>`:'No mobile'}</small></td><td>₹${Number(r.total||0).toLocaleString('en-IN')}<br><small>${esc(r.order_status||'')} · ${esc(r.payment_method||'')}</small></td><td>${esc(r.reason)}<br><small>${esc(r.request_type||'REPLACEMENT')}${r.replacement_size?` · Size ${esc(r.replacement_size)}`:''}${r.replacement_color?` · ${esc(r.replacement_color)}`:''}</small>${r.replacement_order_id?`<br><small><b>📦 Replacement #${r.replacement_order_id}</b> · ${esc(String(r.replacement_order_status||'').replaceAll('_',' '))}<br>Same address: ${esc(r.replacement_address||'')}</small>`:''}</td><td><select id="return-status-${r.id}" class="status-select">${statusOptions.map(st=>`<option ${st===r.status?'selected':''}>${st}</option>`).join('')}</select><br><input id="return-pickup-${r.id}" type="datetime-local" value="${esc((r.pickup_at||'').replace(' ','T'))}" style="margin-top:6px;padding:7px;border:1px solid #d8c7cc;border-radius:7px;width:190px"><br><textarea id="return-note-${r.id}" placeholder="Admin note (optional)" style="margin-top:6px;padding:7px;border:1px solid #d8c7cc;border-radius:7px;width:190px;min-height:55px">${esc(r.admin_note||'')}</textarea><br><small>Return requested: ${new Date(r.created_at).toLocaleString('en-IN')}</small><br><small>Order placed: ${r.order_date?new Date(r.order_date).toLocaleString('en-IN'):'—'}</small>${r.order_status==='DELIVERED'&&r.order_updated_at?`<br><small>Delivered: ${new Date(r.order_updated_at).toLocaleString('en-IN')}</small>`:''}</td><td><button class="gold" type="button" onclick="saveReturnAdmin(${r.id})">Save Status</button>${refundAction}</td></tr>`}).join('')||'<tr><td colspan="6">No return requests yet.</td></tr>';
  openM(`<h2>↩️ Return Management</h2><div class="admin-note">Customers keep their normal replacement/exchange option. Use <b>Open Return for Customer</b> only when you personally approve a return and need to issue a Razorpay refund. Refund unlocks only after the item is received and passes inspection.</div><div class="admin-toolbar"><button class="gold" type="button" onclick="openAdminReturnForCustomer()">＋ Open Return for Customer</button><button type="button" onclick="dashboard()">Back to Dashboard</button><button class="gold" type="button" onclick="adminReturns()">↻ Refresh</button></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Request</th><th>Customer</th><th>Order</th><th>Reason / Option</th><th>Status & Pickup</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table></div>`)
 }catch(e){alert(e.message)}
}
...
async function refundReturn(id){const amount=Number(document.getElementById(`return-refund-${id}`)?.value||0);if(!Number.isFinite(amount)||amount<=0){toast('Enter a valid refund amount');return}if(!confirm(`Start a Razorpay refund of ₹${amount.toLocaleString('en-IN')}? This cannot be undone.`))return;try{await api(`/api/admin/returns/${id}/refund`,{method:'POST',body:{amount}});toast('✓ Refund initiated and customer notified');adminReturns()}catch(e){alert(e.message||'Refund could not be initiated')}}
async function openAdminReturnForCustomer(){
 if(user?.role!=='admin')return alert('Admin only');
 try{
  const orders=await api('/api/admin/orders');
  const eligible=(orders||[]).filter(o=>String(o.status||'').toUpperCase()==='DELIVERED'&&String(o.payment_method||'').toUpperCase()==='RAZORPAY'&&String(o.payment_status||'').toUpperCase()==='PAID'&&o.razorpay_payment_id);
  if(!eligible.length){alert('No delivered, paid Razorpay orders are currently available for an admin return.');return}
  const options=eligible.map(o=>`<option value="${Number(o.id)}">Order #${Number(o.id)} — ${esc(o.name||o.customer_name||'Customer')} — ₹${Number(o.total||0).toLocaleString('en-IN')}</option>`).join('');
  openM(`<h2>↩️ Open Return for Customer</h2><p class="admin-note">This is an admin-only action. It opens a return for one selected customer; customers do not get a new public Return button.</p><div class="admin-form"><label>Delivered paid Razorpay order<select id="admin_return_order">${options}</select></label><label>Reason shown to customer<textarea id="admin_return_reason" placeholder="Example: Store-approved return after delivery issue">Store-approved return</textarea></label></div><div class="admin-actions"><button class="admin-danger" type="button" onclick="createAdminReturnForCustomer()">Open Return</button><button type="button" onclick="adminReturns()">Cancel</button></div>`);
 }catch(e){alert(e.message||'Could not load customer orders')}
}
async function createAdminReturnForCustomer(){
 const orderId=Number(document.getElementById('admin_return_order')?.value||0),reason=document.getElementById('admin_return_reason')?.value||'';
 if(!orderId)return toast('Choose a customer order');
 if(!confirm('Open a return for this customer? You can schedule pickup and issue a refund later.'))return;
 try{await api('/api/admin/returns',{method:'POST',body:{order_id:orderId,reason}});toast('✓ Return opened and customer notified');adminReturns()}catch(e){alert(e.message||'Could not open the customer return')}
}
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 restoreSession().finally(()=>{applyAppearance();loadSiteLogo();load();loadSlides();loadShopCategories();loadQuickFilters();const savedPin=localStorage.getItem('ashwiniDeliveryPin')||'',mobilePin=document.getElementById('mobileDeliveryPin');if(savedPin&&mobilePin){mobilePin.value=savedPin;lookupDeliveryPin(savedPin)}updateHelpUnreadBadge();if(window.__helpUnreadTimer)clearInterval(window.__helpUnreadTimer);window.__helpUnreadTimer=setInterval(updateHelpUnreadBadge,2500);setTimeout(showOfferPopup,1200);setTimeout(()=>{warmMsg91().catch(()=>{})},400);});
});
