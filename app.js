...
async function advanceAdminOrder(id,nextStatus){const booking=nextStatus==='PACKED',question=booking?`Mark Order #${id} PACKED and create its Shiprocket shipment, AWB, pickup and label? Wallet charges may apply.`:`Accept and confirm Order #${id}?`;if(!confirm(question))return;try{await api(`/api/admin/orders/${Number(id)}`,{method:'PATCH',body:{status:nextStatus}});toast(booking?`✓ Order #${id} packed and courier booking prepared`:`✓ Order #${id} accepted and confirmed`);await adminStat('orders')}catch(e){alert((booking?'Courier booking failed: ':'Order confirmation failed: ')+(e.message||'Please try again'))}}
