...
async function returnBookingPreview(id){
 ...
 const items=db.prepare('SELECT oi.*,p.name,p.packed_weight_kg,p.packed_length_cm,p.packed_breadth_cm,p.packed_height_cm FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?').all(o.id),body=returnPayload(r,o,items,destination),destinationKey=crypto.createHash('sha256').update(JSON.stringify(body)).digest('hex');return {body,destination,destinationKey};
}
