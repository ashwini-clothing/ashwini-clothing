...
app.post("/api/checkout/create",auth,async(req,res)=>{
 ...
 try{
  ...
  const firstOrder=db.prepare("SELECT COUNT(*) n FROM orders WHERE user_id=? AND (payment_method<>'RAZORPAY' OR payment_status NOT IN ('PENDING','FAILED','CANCELLED') OR status='PAYMENT_REVIEW')").get(currentUser.id).n===0;
