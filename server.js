...
function createSession(req,res,userId){const raw=crypto.randomBytes(32).toString("base64url"),hash=sessionHash(raw),now=Date.now(),exp=now+sessionTtlMs,ua=String(req?.headers?.['user-agent']||'').slice(0,500);const create=db.transaction(()=>{db.prepare("DELETE FROM auth_sessions WHERE expires_at<?").run(now);db.prepare("INSERT INTO auth_sessions(session_hash,user_id,expires_at,last_seen_at,user_agent,device_label) VALUES(?,?,?,?,?,?)").run(hash,userId,exp,now,ua,sessionDeviceLabel(ua));db.prepare("DELETE FROM auth_sessions WHERE user_id=? AND id NOT IN (SELECT id FROM auth_sessions WHERE user_id=? ORDER BY last_seen_at DESC,id DESC LIMIT ?)").run(userId,userId,maxSessionsPerUser)});create();setSessionCookie(res,raw);return raw}
...
app.get('/api/me/personal-data',auth,(req,res)=>{try{const id=req.user.id,profile=db.prepare('SELECT id,name,email,phone,role,two_step_enabled,two_step_channel,created_at FROM users WHERE id=?').get(id),orders=db.prepare('SELECT * FROM orders WHERE user_id=? ORDER BY id').all(id).map(order=>({...order,items:db.prepare('SELECT oi.product_id,p.name,oi.unit_price AS price,oi.quantity,oi.size FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?').all(order.id)})),data={exported_at:new Date().toISOString(),profile,orders,returns:db.prepare('SELECT * FROM returns WHERE user_id=? ORDER BY id').all(id),reviews:db.prepare('SELECT product_id,rating,feedback,created_at FROM product_reviews WHERE user_id=? ORDER BY id').all(id),questions:db.prepare('SELECT product_id,question,created_at FROM product_questions WHERE user_id=? ORDER BY id').all(id),support_requests:db.prepare('SELECT subject,message,contact_method,status,created_at,updated_at FROM customer_help_requests WHERE user_id=? ORDER BY id').all(id),behavior_events:db.prepare('SELECT event_type,product_id,context_product_id,metadata,created_at FROM behavior_events WHERE user_id=? ORDER BY id').all(id)};res.setHeader('Content-Disposition',`attachment; filename="ashwini-personal-data-${id}.json"`);res.json(data)}catch(e){res.status(500).json({error:'Could not prepare your personal data'})}});
app.post('/api/me/account-deletion-request',auth,async(req,res)=>{try{if(req.user.role==='admin')return res.status(400).json({error:'The store admin account cannot be deleted from the customer page'});const password=String(req.body?.password||''),reason=String(req.body?.reason||'').trim().slice(0,500),account=db.prepare('SELECT password_hash FROM users WHERE id=?').get(req.user.id);if(!account||!(await bcrypt.compare(password,account.password_hash)))return res.status(401).json({error:'Password is incorrect'});const pending=db.prepare("SELECT id FROM account_deletion_requests WHERE user_id=? AND status='PENDING'").get(req.user.id);if(pending)return res.status(409).json({error:'Your account deletion request is already pending'});const result=db.prepare("INSERT INTO account_deletion_requests(user_id,reason) VALUES(?,?)").run(req.user.id,reason);await Promise.allSettled([sendEmail(req.user.email,'Ashwini account deletion request received',`We received your account deletion request #${result.lastInsertRowid}. Your account has not been deleted yet. Ashwini Support will review pending orders, payments, returns and legally required records before completing it.`),sendEmail(adminEmail(),`Ashwini account deletion request #${result.lastInsertRowid}`,`Customer: ${req.user.name} (${req.user.email})\nReason: ${reason||'Not provided'}\nPlease review pending orders, payments and returns before processing this request.`)]);res.status(201).json({ok:true,id:result.lastInsertRowid,message:'Account deletion request submitted. Your account remains active until review is complete.'})}catch(e){res.status(500).json({error:'Could not submit account deletion request'})}});
