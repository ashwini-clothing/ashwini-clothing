import {createReturnRefundService} from './scripts/return-refund.js';
import {installReturnCourierSchema,returnCourierLinked,returnPayload,applyReturnWebhook,createReturnCourierService} from './scripts/return-courier.js';
...
function installPermanentProductDelete(){
 const columns=db.prepare('PRAGMA table_info(order_items)').all();
 if(columns.find(c=>c.name==='product_id')?.notnull){
  db.transaction(()=>{
   const definition=db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='order_items'").get().sql;
   const indexes=db.prepare("SELECT sql FROM sqlite_master WHERE tbl_name='order_items' AND type IN ('index','trigger') AND sql IS NOT NULL").all();
   const revised=definition.replace(/CREATE TABLE\s+(?:IF NOT EXISTS\s+)?["`]?order_items["`]?/i,'CREATE TABLE order_items_delete_migration').replace(/product_id\s+INTEGER\s+NOT NULL/i,'product_id INTEGER');
   if(revised===definition||/product_id\s+INTEGER\s+NOT NULL/i.test(revised))throw Error('Order item migration could not be prepared');
   db.exec(revised);
   const names=columns.map(c=>'"'+c.name.replaceAll('"','""')+'"').join(',');
   db.exec(`INSERT INTO order_items_delete_migration (${names}) SELECT ${names} FROM order_items; DROP TABLE order_items; ALTER TABLE order_items_delete_migration RENAME TO order_items;`);
   for(const index of indexes)db.exec(index.sql);
  })();
 }
 const current=db.prepare('PRAGMA table_info(order_items)').all();
 for(const name of ['deleted_product_name','deleted_product_image','deleted_product_emoji'])if(!current.some(c=>c.name===name))db.exec(`ALTER TABLE order_items ADD COLUMN ${name} TEXT`);
 db.exec(`CREATE TABLE IF NOT EXISTS permanently_deleted_products(id INTEGER PRIMARY KEY,deleted_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP);
 CREATE TRIGGER IF NOT EXISTS prevent_deleted_product_reseed BEFORE INSERT ON products WHEN EXISTS(SELECT 1 FROM permanently_deleted_products WHERE id=NEW.id) BEGIN SELECT RAISE(IGNORE); END;`);
}
installPermanentProductDelete();

function permanentlyDeleteProduct(id){
 return db.transaction(()=>{
  const product=db.prepare('SELECT * FROM products WHERE id=?').get(id);
  if(!product)return null;
  db.prepare('UPDATE order_items SET deleted_product_name=?,deleted_product_image=?,deleted_product_emoji=?,product_id=NULL WHERE product_id=?').run(product.name,product.image||'',product.emoji||'👗',id);
  db.prepare('INSERT OR IGNORE INTO permanently_deleted_products(id) VALUES(?)').run(id);
...
for(const [column,type] of [['delivery_name','TEXT DEFAULT \'\''],['delivery_address_line','TEXT DEFAULT \'\''],['delivery_city','TEXT DEFAULT \'\''],['delivery_state','TEXT DEFAULT \'\''],['delivery_pincode','TEXT DEFAULT \'\''],['shiprocket_order_id','TEXT DEFAULT \'\''],['shiprocket_shipment_id','TEXT DEFAULT \'\''],['shiprocket_awb','TEXT DEFAULT \'\''],['shiprocket_courier_id','TEXT DEFAULT \'\''],['shiprocket_status','TEXT DEFAULT \'\''],['shiprocket_label_url','TEXT DEFAULT \'\''],['shipment_weight_kg','REAL DEFAULT 0'],['shipment_length_cm','REAL DEFAULT 0'],['shipment_breadth_cm','REAL DEFAULT 0'],['shipment_height_cm','REAL DEFAULT 0']])try{db.exec(`ALTER TABLE orders ADD COLUMN ${column} ${type}`)}catch{}
for(const [column,type] of [['packed_weight_kg','REAL NOT NULL DEFAULT 0.5'],['packed_length_cm','REAL NOT NULL DEFAULT 25'],['packed_breadth_cm','REAL NOT NULL DEFAULT 20'],['packed_height_cm','REAL NOT NULL DEFAULT 5']])try{db.exec(`ALTER TABLE products ADD COLUMN ${column} ${type}`)}catch{}
...
try{db.exec(`CREATE TABLE IF NOT EXISTS cart_items (user_id INTEGER NOT NULL,product_id INTEGER NOT NULL,size TEXT NOT NULL,quantity INTEGER NOT NULL CHECK(quantity BETWEEN 1 AND 20),updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,PRIMARY KEY(user_id,product_id,size),FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(product_id) REFERENCES products(id) ON DELETE CASCADE)`)}catch(e){console.error('[Cart table]',e.message)}
...
try{db.exec(`CREATE TABLE IF NOT EXISTS saved_delivery_addresses (user_id INTEGER PRIMARY KEY,name TEXT NOT NULL,phone TEXT NOT NULL,address_line TEXT NOT NULL,city TEXT NOT NULL,state TEXT NOT NULL,pincode TEXT NOT NULL,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)`)}catch(e){console.error('[Saved delivery address]',e.message)}
if(!db.prepare('PRAGMA table_info(saved_delivery_addresses)').all().some(x=>x.name==='address_parts'))db.exec("ALTER TABLE saved_delivery_addresses ADD COLUMN address_parts TEXT NOT NULL DEFAULT ''");
...
try{db.exec(`CREATE TABLE IF NOT EXISTS shiprocket_webhook_events (event_hash TEXT PRIMARY KEY,awb TEXT NOT NULL DEFAULT '',shipment_status TEXT NOT NULL DEFAULT '',received_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`)}catch(e){console.error('[Shiprocket webhook table]',e.message)}
...
db.prepare("UPDATE products SET image=? WHERE id=? AND COALESCE(image,'')=''").run('/new-model-dress-clean.jpg',100);
...
async function cancelOrderSafely(order,{customerRequest=false}={}){
 order=db.prepare('SELECT * FROM orders WHERE id=?').get(order.id);
 if(!order)throw Error('Order not found');
 if(customerRequest&&order.status!=='CANCELLED'&&!['PAYMENT_PENDING','PAYMENT_EXPIRED','PAYMENT_FAILED','PLACED'].includes(String(order.status)))throw Error('Admin ne order confirm kar diya hai. Ab customer is order ko cancel nahi kar sakta.');
 if(['PROCESSING','UNKNOWN'].includes(order.refund_status))throw Error('Refund confirmation is pending. Check Razorpay before trying again.');
 if(order.status!=='CANCELLED'&&(courierBookingLocks.has(order.id)||((order.shiprocket_shipment_id||order.shiprocket_order_id)&&!['CANCELLED','CANCELED'].includes(String(order.shiprocket_status).toUpperCase()))))throw Error('Courier booking exists or is in progress. Cancel it with the courier and verify pickup has not happened before cancelling this order.');
 ...
 if(order.payment_method==='RAZORPAY'&&order.payment_status==='PAID'){
  ...
  }catch(error){db.prepare("UPDATE orders SET refund_status='UNKNOWN',updated_at=CURRENT_TIMESTAMP WHERE id=? AND refund_status='PROCESSING'").run(order.id);throw Error(`Refund confirmation is unclear; check Razorpay before retrying: ${error?.error?.description||error.message||'Razorpay error'}`)}
 }else{
...
app.post('/api/webhooks/razorpay',express.raw({type:'application/json',limit:'1mb'}),(req,res)=>{
 ...
 try{
  ...
  const order=razorpayOrderId?db.prepare("SELECT * FROM orders WHERE razorpay_order_id=? AND payment_method='RAZORPAY'").get(razorpayOrderId):db.prepare("SELECT * FROM orders WHERE payment_method='RAZORPAY' AND ((razorpay_payment_id=? AND razorpay_payment_id<>'') OR (razorpay_refund_id=? AND razorpay_refund_id<>''))").get(relatedPaymentId,String(refund?.id||''));
  ...
  if(refundEvents.includes(eventType)){
   ...
   returnRefund.apply({...refund,status:refundStatus.toLowerCase()});
  ...
  if(eventType==='payment.failed'){
   if(['PENDING','FAILED'].includes(order.payment_status)){db.prepare("UPDATE orders SET payment_status='FAILED',status=CASE WHEN status IN ('PAYMENT_PENDING','PAYMENT_EXPIRED') THEN 'PAYMENT_FAILED' ELSE status END,razorpay_payment_id=COALESCE(NULLIF(?,''),razorpay_payment_id),updated_at=CURRENT_TIMESTAMP WHERE id=?").run(String(payment?.id||''),order.id);releaseOrderStock(order.id,'PAYMENT_FAILED')}
  }else{
   if(['REFUNDED','PARTIALLY_REFUNDED','REFUND_PENDING','REFUND_FAILED','DISPUTED','DISPUTE_LOST'].includes(order.payment_status)||order.status==='CANCELLED'){finish('IGNORED','Order is already cancelled, refunded or disputed');return res.json({ok:true,ignored:true})}
...
function publicOtpResponse(otp,channel,message){const dev=process.env.NODE_ENV!=="production";return {ok:true,channel,message,...(dev?{devOtp:otp}:{})};}
...
async function prepareShiprocketShipment(orderId,pickupDate){
 ...
 const items=db.prepare(`SELECT oi.product_id,oi.size,oi.quantity,oi.unit_price,COALESCE(p.name,oi.deleted_product_name) AS name,p.packed_weight_kg,p.packed_length_cm,p.packed_breadth_cm,p.packed_height_cm FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?`).all(order.id);if(!items.length)throw Error('Order has no shipment items');
 const fallback={weight:Math.max(.05,Number(process.env.SHIPROCKET_DEFAULT_WEIGHT_KG)||.5),length:Math.max(1,Number(process.env.SHIPROCKET_DEFAULT_LENGTH_CM)||25),breadth:Math.max(1,Number(process.env.SHIPROCKET_DEFAULT_BREADTH_CM)||20),height:Math.max(1,Number(process.env.SHIPROCKET_DEFAULT_HEIGHT_CM)||5)},measure=(value,otherwise)=>Number(value)>0?Number(value):otherwise;
 const parcel={weight:items.reduce((n,x)=>n+measure(x.packed_weight_kg,fallback.weight)*Number(x.quantity||0),0),length:Math.max(...items.map(x=>measure(x.packed_length_cm,fallback.length))),breadth:Math.max(...items.map(x=>measure(x.packed_breadth_cm,fallback.breadth))),height:items.reduce((n,x)=>n+measure(x.packed_height_cm,fallback.height)*Number(x.quantity||0),0)};
 parcel.weight=Number(Math.max(.05,parcel.weight).toFixed(3));parcel.length=Number(Math.max(1,parcel.length).toFixed(1));parcel.breadth=Number(Math.max(1,parcel.breadth).toFixed(1));parcel.height=Number(Math.max(1,parcel.height).toFixed(1));
 db.prepare('UPDATE orders SET shipment_weight_kg=?,shipment_length_cm=?,shipment_breadth_cm=?,shipment_height_cm=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(parcel.weight,parcel.length,parcel.breadth,parcel.height,order.id);
 let shipmentId=String(order.shiprocket_shipment_id||''),shiprocketOrderId=String(order.shiprocket_order_id||'');
 if(!shipmentId){
  const pickup=await shiprocketPickupLocation(),shipmentValue=Math.max(Number(order.total)||0,items.reduce((n,x)=>n+Number(x.unit_price||0)*Number(x.quantity||0),0)),created=await shiprocketRequest('/orders/create/adhoc',{method:'POST',body:{order_id:`ASH-${order.id}`,order_date:new Date(order.created_at||Date.now()).toISOString().slice(0,19).replace('T',' '),pickup_location:pickup,billing_customer_name:String(order.delivery_name||order.customer_name||'Customer').slice(0,100),billing_last_name:'',billing_address:String(order.delivery_address_line).slice(0,200),billing_city:String(order.delivery_city).slice(0,100),billing_pincode:String(order.delivery_pincode),billing_state:String(order.delivery_state).slice(0,100),billing_country:'India',billing_email:String(order.customer_email||'').slice(0,150),billing_phone:String(order.customer_phone||'').replace(/\D/g,'').slice(-10),shipping_is_billing:true,order_items:items.map(x=>({name:String(x.name||`Product ${x.product_id}`).slice(0,100),sku:`ASH-${x.product_id}-${String(x.size||'NA').replace(/\s+/g,'-')}`,units:Number(x.quantity),selling_price:Number(x.unit_price),discount:'',tax:'',hsn:''})),payment_method:order.payment_method==='COD'?'COD':'Prepaid',shipping_charges:0,giftwrap_charges:0,transaction_charges:0,total_discount:0,sub_total:shipmentValue,length:parcel.length,breadth:parcel.breadth,height:parcel.height,weight:parcel.weight}});
...
// Persist the first-order promotion separately from general seasonal offers.
db.exec("CREATE TABLE IF NOT EXISTS new_buyer_coupon (id INTEGER PRIMARY KEY CHECK(id=1),code TEXT NOT NULL,discount_percent REAL NOT NULL CHECK(discount_percent>0 AND discount_percent<=100),active INTEGER NOT NULL DEFAULT 1)");
db.prepare("INSERT OR IGNORE INTO new_buyer_coupon(id,code,discount_percent,active) VALUES(1,'NEW2026',30,1)").run();
function getNewBuyerCoupon(){return db.prepare('SELECT code,discount_percent,active FROM new_buyer_coupon WHERE id=1').get()}
app.get('/api/coupons/new-buyer',auth,(req,res)=>res.json(getNewBuyerCoupon()));
app.get('/api/admin/new-buyer-coupon',auth,admin,(req,res)=>res.json(getNewBuyerCoupon()));
app.patch('/api/admin/new-buyer-coupon',auth,admin,(req,res)=>{try{const b=req.body||{},code=String(b.code||'').trim().toUpperCase(),pct=Number(b.discount_percent);if(!/^[A-Z0-9_-]{2,32}$/.test(code))throw Error('Use 2–32 letters, numbers, hyphens or underscores for the code');if(!Number.isFinite(pct)||pct<=0||pct>100)throw Error('Discount must be greater than 0 and at most 100');if(typeof b.active!=='boolean')throw Error('Choose whether the coupon is active');if(db.prepare('SELECT id FROM offers WHERE UPPER(TRIM(coupon_code))=? LIMIT 1').get(code))throw Error('This code is already used by a seasonal offer. Choose another code.');db.prepare('UPDATE new_buyer_coupon SET code=?,discount_percent=?,active=? WHERE id=1').run(code,pct,b.active?1:0);res.json(getNewBuyerCoupon())}catch(e){res.status(400).json({error:e.message})}});
function resolveCheckoutCoupon(code,userId){code=String(code||'').trim().toUpperCase();const first=getNewBuyerCoupon();if(code===first.code){if(!first.active)throw Error('Coupon not recognised or expired');if(!isFirstValidOrderCustomer(userId))throw Error('Coupon already used or not available for this account');return {code,discount_percent:first.discount_percent}}const now=new Date().toISOString(),offer=db.prepare("SELECT * FROM offers WHERE active=1 AND coupon_code=? AND (start_at='' OR start_at<=?) AND (end_at='' OR end_at>=?) ORDER BY id DESC LIMIT 1").get(code,now,now);if(!offer)throw Error('Coupon not recognised or expired');return {code:offer.coupon_code,discount_percent:Math.max(0,Math.min(100,Number(offer.discount_percent)||0)),title:offer.title}}
function isFirstValidOrderCustomer(userId){return db.prepare("SELECT COUNT(*) n FROM orders WHERE user_id=? AND (payment_method<>'RAZORPAY' OR payment_status NOT IN ('PENDING','FAILED','CANCELLED') OR status='PAYMENT_REVIEW')").get(userId).n===0}
app.get('/api/coupons/available',auth,(req,res)=>{try{const now=new Date().toISOString(),rows=db.prepare("SELECT coupon_code AS code,title,discount_percent FROM offers WHERE active=1 AND trim(coupon_code)<>'' AND discount_percent>0 AND (start_at='' OR start_at<=?) AND (end_at='' OR end_at>=?) ORDER BY discount_percent DESC,id DESC").all(now,now).map(x=>({...x,code:String(x.code||'').trim().toUpperCase(),discount_percent:Number(x.discount_percent||0)}));const first=getNewBuyerCoupon(),available=rows.filter(x=>x.code!==first.code);if(first.active&&isFirstValidOrderCustomer(req.user.id))available.unshift({code:first.code,title:'New customer offer',discount_percent:first.discount_percent});res.json(available)}catch(e){res.status(500).json({error:'Available coupons could not be loaded'})}});
app.post("/api/coupons/check",auth,(req,res)=>{try{res.json({ok:true,...resolveCheckoutCoupon(req.body?.code,req.user.id)})}catch(e){res.status(400).json({error:e.message})}});
...
app.post("/api/auth/register-msg91",async(req,res)=>{
 ...
 try{
  ...
  try{
   ...
   if(!delivery.sent&&process.env.NODE_ENV==='production'){db.prepare('DELETE FROM pending_registrations WHERE token_hash=?').run(sessionHash(registrationToken));return res.status(503).json({error:'Email OTP service is not configured.'})}
...
app.post("/api/auth/request-recovery-otp",async(req,res)=>{
 ...
 try{
  ...
  if(!delivery.sent && process.env.NODE_ENV==='production')return res.status(503).json({error:`${channel==='email'?'Email':'SMS'} OTP service is not configured.`});
...
app.post("/api/auth/reset-password",async(req,res)=>{
 ...
 const resetAccount=db.transaction(()=>{
  const changed=db.prepare("UPDATE users SET password_hash=?,recovery_otp_hash='',recovery_otp_expires_at=0,login_otp_hash='',login_otp_expires_at=0 WHERE id=? AND recovery_otp_hash=? AND recovery_otp_expires_at>=?").run(hash,u.id,u.recovery_otp_hash,Date.now());
  if(!changed.changes)return false;
  db.prepare("DELETE FROM auth_sessions WHERE user_id=?").run(u.id);
  return true;
 });
 if(!resetAccount())return res.status(400).json({error:"Invalid or expired OTP"});
 clearOtpFailures(req,identifier);
...
app.post("/api/auth/admin-login-start",async(req,res)=>{
 try{
  ...
  if(!delivery.sent && process.env.NODE_ENV==='production')return res.status(503).json({error:"Admin Email OTP service is not configured. Please check Render Email environment variables."});
...
app.get('/api/me/personal-data',auth,(req,res)=>{try{
 ...
 const orders=db.prepare(`SELECT id,total,status,payment_status,payment_method,razorpay_payment_id AS payment_reference,refund_status,refund_amount,dispute_status,address,customer_phone,created_at,updated_at,delivered_at,cancelled_at FROM orders WHERE user_id=? ORDER BY id`).all(id).map(order=>({...order,items:db.prepare('SELECT oi.product_id,COALESCE(p.name,oi.deleted_product_name) AS name,oi.unit_price AS price,oi.quantity,oi.size FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?').all(order.id),events:db.prepare('SELECT status,title,message,created_at FROM order_events WHERE order_id=? AND user_id=? ORDER BY id').all(order.id,id)}));
...
function saveDeliveryAddressFromOrder(userId,order){db.prepare(`INSERT INTO saved_delivery_addresses(user_id,name,phone,address_line,city,state,pincode,updated_at) VALUES(?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET address_parts=CASE WHEN saved_delivery_addresses.address_line=excluded.address_line THEN saved_delivery_addresses.address_parts ELSE '' END,name=excluded.name,phone=excluded.phone,address_line=excluded.address_line,city=excluded.city,state=excluded.state,pincode=excluded.pincode,updated_at=CURRENT_TIMESTAMP`).run(userId,order.delivery_name,order.customer_phone,order.delivery_address_line,order.delivery_city,order.delivery_state,order.delivery_pincode)}
app.post("/api/checkout/create",auth,async(req,res)=>{
 let createdOrderId=null;
 try{
  const {items,payment_method:rawPaymentMethod="RAZORPAY",coupon="",idempotency_key=""}=req.body||{};const payment_method=String(rawPaymentMethod).trim().toUpperCase();
  ...
  if(couponCode){const offer=resolveCheckoutCoupon(couponCode,currentUser.id);total=Math.round(total*(1-offer.discount_percent/100));}
  ...
  if(payment_method==="COD"){saveDeliveryAddressFromOrder(currentUser.id,db.prepare('SELECT * FROM orders WHERE id=?').get(orderId));return res.json({ok:true,mode:"COD",orderId,total})}
...
app.post("/api/checkout/verify",auth,async(req,res)=>{
 try{
  ...
  res.json({ok:true});
 }catch(e){console.error('[Razorpay checkout verification]',e.message);res.status(400).json({error:'Payment could not be verified. No order has been placed.'})}
});
...
app.get('/api/me/delivery-address',auth,(req,res)=>res.json({address:readAddressParts(db.prepare('SELECT name,phone,address_line,address_parts,city,state,pincode,updated_at FROM saved_delivery_addresses WHERE user_id=?').get(req.user.id))}));
app.put('/api/me/delivery-address',auth,async(req,res)=>{try{const structured=req.body?.parts===undefined?null:normalizeAddressParts(req.body.parts);const name=String(req.body?.name||'').trim(),phone=String(req.body?.phone||'').replace(/\D/g,''),addressLine=structured?structured.line:String(req.body?.address_line||'').trim(),city=String(req.body?.city||'').trim(),state=String(req.body?.state||'').trim(),pincode=String(req.body?.pincode||'').trim();if(name.length<2||name.length>100)throw Error('Enter the customer full name');if(!/^[6-9]\d{9}$/.test(phone))throw Error('Enter a valid 10-digit Indian mobile number');if(addressLine.length<8||addressLine.length>500)throw Error('Enter a complete delivery address');if(!/^\d{6}$/.test(pincode))throw Error('Enter a valid 6-digit PIN code');const place=await lookupPostalPin(pincode);if(!place)throw Error('PIN code location could not be verified');const norm=v=>String(v||'').trim().toLowerCase().replace(/[^a-z0-9]+/g,' '),validCities=[place.city,place.district].map(norm).filter(Boolean);if(!validCities.includes(norm(city))||norm(state)!==norm(place.state))throw Error('City or state does not match the PIN code');db.prepare(`INSERT INTO saved_delivery_addresses(user_id,name,phone,address_line,city,state,pincode,address_parts,updated_at) VALUES(?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET address_parts=excluded.address_parts,name=excluded.name,phone=excluded.phone,address_line=excluded.address_line,city=excluded.city,state=excluded.state,pincode=excluded.pincode,updated_at=CURRENT_TIMESTAMP`).run(req.user.id,name,phone,addressLine,city,state,pincode,structured?JSON.stringify(structured.parts):'');res.json({ok:true,address:readAddressParts(db.prepare('SELECT name,phone,address_line,address_parts,city,state,pincode,updated_at FROM saved_delivery_addresses WHERE user_id=?').get(req.user.id))})}catch(e){res.status(400).json({error:e.message})}});
...
app.get('/api/order-events',auth,(req,res)=>{try{res.json(db.prepare("SELECT e.* FROM order_events e JOIN orders o ON o.id=e.order_id WHERE e.user_id=? AND (o.payment_method<>'RAZORPAY' OR o.payment_status NOT IN ('PENDING','FAILED','CANCELLED') OR o.status='PAYMENT_REVIEW') ORDER BY e.id DESC LIMIT 100").all(req.user.id))}catch(e){res.status(400).json({error:e.message})}});
...
function customerReturnView(row){return Object.fromEntries(Object.entries(row).filter(([key])=>!key.startsWith('sr_')))}
app.get('/api/returns',auth,(req,res)=>{try{res.json(db.prepare(`SELECT r.*,o.total,o.status AS order_status,o.created_at AS order_date,ro.status AS replacement_order_status,ro.address AS replacement_address FROM returns r JOIN orders o ON o.id=r.order_id LEFT JOIN orders ro ON ro.id=r.replacement_order_id WHERE r.user_id=? ORDER BY r.id DESC`).all(req.user.id).map(r=>returnRefund.view(customerReturnView(r))))}catch(e){res.status(400).json({error:e.message})}});
installReturnCourierSchema(db);
const returnRefund=createReturnRefundService({db,provider:razorpay?.payments,addEvent:addReturnEvent});
const returnCourier=createReturnCourierService({db,request:shiprocketRequest,addEvent:addReturnEvent});
async function returnBookingPreview(id){
 const r=db.prepare('SELECT * FROM returns WHERE id=?').get(id);if(!r||r.status!=='APPROVED')throw Error('Approve this return first');
 const o=db.prepare('SELECT o.*,u.name AS customer_name,u.email AS customer_email FROM orders o JOIN users u ON u.id=o.user_id WHERE o.id=?').get(r.order_id);
 const response=await shiprocketRequest('/settings/company/pickup'),locations=response?.data?.shipping_address||response?.shipping_address||[],name=await shiprocketPickupLocation(),location=locations.find(x=>String(x.pickup_location)===name);if(!location)throw Error('Configured Shiprocket return warehouse was not found. Check the pickup location setting.');
 const destination={name:location.name||location.pickup_location,address:[location.address,location.address_2].filter(Boolean).join(', '),city:location.city,state:location.state,pin:location.pin_code||location.pincode,phone:location.phone,email:location.email};
 const items=db.prepare('SELECT oi.*,COALESCE(p.name,oi.deleted_product_name) AS name,p.packed_weight_kg,p.packed_length_cm,p.packed_breadth_cm,p.packed_height_cm FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?').all(o.id),body=returnPayload(r,o,items,destination),destinationKey=crypto.createHash('sha256').update(JSON.stringify(body)).digest('hex');return {body,destination,destinationKey};
}
app.get('/api/admin/returns/:id/courier-preview',auth,admin,async(req,res)=>{try{const d=await returnBookingPreview(Number(req.params.id));res.json({destination:d.destination,destinationKey:d.destinationKey})}catch(e){res.status(409).json({error:e.message})}});
app.post('/api/admin/returns/:id/book-courier',auth,admin,async(req,res)=>{try{const id=Number(req.params.id),d=await returnBookingPreview(id);if(req.body.destinationKey!==d.destinationKey)return res.status(409).json({error:'Return details changed. Reopen the booking preview before confirming.'});const r=await returnCourier.book(id,d.body);logAdminActivity(req,'RETURN_COURIER_BOOKED','RETURN',id,{shipment_id:r.sr_shipment_id});res.json({ok:true,return:r})}catch(e){res.status(409).json({error:e.message})}});
app.post('/api/admin/returns/:id/courier-label',auth,admin,async(req,res)=>{try{res.json({url:await returnCourier.label(Number(req.params.id))})}catch(e){res.status(409).json({error:e.message})}});
app.post('/api/admin/returns/:id/sync-courier',auth,admin,async(req,res)=>{try{const r=db.prepare('SELECT * FROM returns WHERE id=?').get(Number(req.params.id));if(!r?.sr_awb)throw Error('No saved return AWB. Check reference ASH-RET-'+req.params.id+' in Shiprocket.');const result=await shiprocketRequest('/courier/track/awb/'+encodeURIComponent(r.sr_awb)),track=result.tracking_data,shipment=track?.shipment_track?.[0];if(!shipment?.current_status)throw Error('Shiprocket has not supplied a tracking status yet');const payload={awb:r.sr_awb,current_status:shipment.current_status};applyReturnWebhook(db,payload,crypto.createHash('sha256').update(JSON.stringify(payload)).digest('hex'),addReturnEvent);res.json({ok:true,return:db.prepare('SELECT * FROM returns WHERE id=?').get(r.id)})}catch(e){res.status(409).json({error:e.message})}});
app.patch('/api/returns/:id/cancel',auth,async(req,res)=>{try{const r=db.prepare(`SELECT r.*,u.name AS customer_name,u.email AS customer_email,o.id AS order_id FROM returns r JOIN users u ON u.id=r.user_id JOIN orders o ON o.id=r.order_id WHERE r.id=? AND r.user_id=?`).get(req.params.id,req.user.id);if(!r)return res.status(404).json({error:'Return request not found'});if(returnCourierLinked(r))return res.status(409).json({error:'Courier return booking exists. Contact Ashwini to cancel it with Shiprocket first.'});const cancellable=['REQUESTED','APPROVED','PICKUP_SCHEDULED'];if(!cancellable.includes(String(r.status)))return res.status(400).json({error:'This return can no longer be cancelled because the pickup/inspection process has started'});db.prepare(`UPDATE returns SET status='CANCELLED',updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(r.id);addReturnEvent(r.id,r.user_id,'CANCELLED','Return request cancelled','Your return request #'+r.id+' for Order #'+r.order_id+' was cancelled by you.');const adminEmail=db.prepare('SELECT email FROM store_profile WHERE id=1').get()?.email||'ashwiniweb88@gmail.com';await Promise.all([sendReturnEmail(adminEmail,`Ashwini Return #${r.id} Cancelled`,`Customer: ${r.customer_name} (${r.customer_email})\nOrder: #${r.order_id}\nThe customer cancelled the return request.`),sendReturnEmail(r.customer_email,`Ashwini Clothing Return Cancelled - Order #${r.order_id}`,`Your return request #${r.id} has been cancelled successfully. Your order remains delivered.`)]);res.json({ok:true,return:db.prepare('SELECT * FROM returns WHERE id=?').get(r.id)})}catch(e){console.error('[Ashwini return cancel]',e);res.status(400).json({error:e.message})}});
app.get('/api/admin/returns',auth,admin,(req,res)=>{try{res.json(db.prepare(`SELECT r.*,o.payment_method,o.payment_status,o.total,o.created_at AS order_date,o.updated_at AS order_updated_at,o.status AS order_status,u.name AS customer_name,u.email AS customer_email,u.phone AS customer_phone,ro.id AS replacement_order_id,ro.status AS replacement_order_status,ro.address AS replacement_address FROM returns r JOIN orders o ON o.id=r.order_id LEFT JOIN users u ON u.id=r.user_id LEFT JOIN orders ro ON ro.id=r.replacement_order_id ORDER BY CASE r.status WHEN 'REQUESTED' THEN 0 WHEN 'APPROVED' THEN 1 WHEN 'PICKUP_SCHEDULED' THEN 2 WHEN 'COMPLETED' THEN 3 ELSE 4 END, r.id DESC`).all().map(r=>returnRefund.view(r,true)))}catch(e){res.status(400).json({error:e.message})}});
...
app.post('/api/admin/returns/:id/refund/:action',auth,admin,async(req,res)=>{try{
 const id=Number(req.params.id),action=req.params.action;if(!Number.isSafeInteger(id)||id<=0)return res.status(400).json({error:'Invalid return'});
 if(!['start','sync','manual'].includes(action))return res.status(404).json({error:'Unknown refund action'});
 if(action==='manual'&&req.body?.transfer_completed!==true)return res.status(400).json({error:'Confirm the actual transfer has completed first'});
 logAdminActivity(req,`RETURN_REFUND_${action.toUpperCase()}_REQUESTED`,'RETURN',id,{});
 const result=action==='start'?await returnRefund.start(id,req.user.id):action==='sync'?await returnRefund.sync(id):returnRefund.manual(id,req.user.id,req.body?.reference);
 logAdminActivity(req,`RETURN_REFUND_${action.toUpperCase()}`,'RETURN',id,{state:result.state,amount_paise:result.amount_paise});
 res.json({ok:true,state:result.state});
}catch(e){res.status(409).json({error:e.message||'Refund could not be processed'})}});
...
app.patch('/api/admin/returns/:id',auth,admin,async(req,res)=>{try{
 ...
 if(returnRefund.job(r.id))return res.status(409).json({error:'Refund has started. Use Sync Refund; do not change return status manually.'});
 if(status==='COMPLETED'&&r.request_type==='RETURN_REFUND')return res.status(409).json({error:'Use Refund to Original Payment or Record COD Refund after inspection. Completion follows refund confirmation.'});
 if(returnCourierLinked(r)&&['CANCELLED','REJECTED','APPROVED','REQUESTED','PICKUP_SCHEDULED','PICKUP_ATTEMPTED','PICKED_UP','IN_TRANSIT','RECEIVED'].includes(status)&&status!==r.status)return res.status(409).json({error:'Use Sync Courier for shipment stages. Cancel a booked return in Shiprocket first, then sync here.'});
 if(['INSPECTION_PASSED','INSPECTION_FAILED'].includes(status)&&!['RECEIVED','INSPECTION_PASSED','INSPECTION_FAILED'].includes(r.status))return res.status(409).json({error:'Receive the returned parcel before inspection.'});
 if(status==='COMPLETED'&&r.status!=='INSPECTION_PASSED')return res.status(409).json({error:'Complete a return only after inspection has passed.'});
 ...
 if(['REQUESTING','UNKNOWN','REVIEW'].includes(r.sr_booking_state)&&!['RECEIVED','INSPECTION_PASSED','INSPECTION_FAILED'].includes(r.status))return res.status(409).json({error:'Resolve this return booking in Shiprocket before changing its status.'});
 let replacementOrderId=r.replacement_order_id||null;
 db.transaction(()=>{
 db.prepare('UPDATE returns SET status=?,pickup_at=?,admin_note=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(status,returnCourierLinked(r)?r.pickup_at:pickupAt,adminNote,req.params.id);
...
function customerOrderView(order){const {razorpay_signature,checkout_key,pickup_token,courier_name,tracking_number,tracking_url,shiprocket_awb,shiprocket_courier_id,shiprocket_order_id,shiprocket_shipment_id,shiprocket_label_url,...customerOrder}=order;return customerOrder}
app.get("/api/orders",auth,(req,res)=>{
 try{
  ...
  const os=db.prepare(`SELECT o.*,u.name AS customer_name,COALESCE(NULLIF(o.customer_phone,''),u.phone) AS customer_phone FROM orders o LEFT JOIN users u ON u.id=o.user_id WHERE o.user_id=? AND (o.payment_method<>'RAZORPAY' OR o.payment_status NOT IN ('PENDING','FAILED','CANCELLED') OR o.status='PAYMENT_REVIEW') ORDER BY o.id DESC`).all(me.id);
  const items=db.prepare("SELECT oi.*,COALESCE(p.name,oi.deleted_product_name) AS name,COALESCE(p.emoji,oi.deleted_product_emoji) AS emoji,COALESCE(p.image,oi.deleted_product_image) AS image FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?");
  const returns=db.prepare('SELECT * FROM returns WHERE order_id=? ORDER BY id DESC');res.json(os.map(o=>{const deliveredAt=o.status==='DELIVERED'?Date.parse(String(o.delivered_at||o.updated_at||o.created_at||'')):NaN;const returnDeadline=Number.isFinite(deliveredAt)?new Date(deliveredAt+4*24*60*60*1000).toISOString():null;return {...customerOrderView(o),return_deadline_at:returnDeadline,return_eligible:Boolean(returnDeadline&&Date.now()<=Date.parse(returnDeadline)),items:items.all(o.id),return_request:(()=>{const rr=returns.get(o.id); if(!rr)return null; const ro=rr.replacement_order_id?db.prepare('SELECT id,status,address FROM orders WHERE id=?').get(rr.replacement_order_id):null; return {...rr,replacement_order:ro||null};})(),tracking:{current:o.status,stages:['PLACED','CONFIRMED','PACKED','SHIPPED','OUT_FOR_DELIVERY','DELIVERED']}};}));
...
app.get("/api/orders/:id",auth,(req,res)=>{
 try{
  const me=db.prepare("SELECT id FROM users WHERE id=?").get(req.user.id);
  if(!me)return res.status(401).json({error:"Customer account was not found. Please sign in again."});
  const o=db.prepare(`SELECT o.*,u.name AS customer_name,COALESCE(NULLIF(o.customer_phone,''),u.phone) AS customer_phone FROM orders o LEFT JOIN users u ON u.id=o.user_id WHERE o.id=? AND o.user_id=? AND (o.payment_method<>'RAZORPAY' OR o.payment_status NOT IN ('PENDING','FAILED','CANCELLED') OR o.status='PAYMENT_REVIEW')`).get(req.params.id,me.id);
  if(!o)return res.status(404).json({error:"Order not found for this account"});
  const items=db.prepare("SELECT oi.*,COALESCE(p.name,oi.deleted_product_name) AS name,COALESCE(p.emoji,oi.deleted_product_emoji) AS emoji,COALESCE(p.image,oi.deleted_product_image) AS image FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=?").all(o.id);
...
function courierBookingPreview(id){
 ...
 const items=db.prepare('SELECT oi.product_id,oi.size,oi.quantity,oi.unit_price,COALESCE(p.name,oi.deleted_product_name) AS name,p.color,p.packed_weight_kg,p.packed_length_cm,p.packed_breadth_cm,p.packed_height_cm FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=? ORDER BY oi.id').all(id);
...
app.get('/api/admin/orders/:id/print-data',auth,admin,(req,res)=>{try{
 ...
 const items=db.prepare(`SELECT oi.product_id,oi.size,oi.quantity,oi.unit_price,COALESCE(p.name,oi.deleted_product_name) AS name,p.color FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=? ORDER BY oi.id`).all(order.id);
...
app.get('/api/admin/returns/:id/print-data',auth,admin,(req,res)=>{try{
 ...
 const items=db.prepare(`SELECT oi.product_id,oi.size,oi.quantity,oi.unit_price,COALESCE(p.name,oi.deleted_product_name) AS name,p.color FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=? ORDER BY oi.id`).all(item.order_id);
 const replacementItems=item.replacement_order_id?db.prepare(`SELECT oi.product_id,oi.size,oi.quantity,oi.unit_price,COALESCE(p.name,oi.deleted_product_name) AS name,p.color FROM order_items oi LEFT JOIN products p ON p.id=oi.product_id WHERE oi.order_id=? ORDER BY oi.id`).all(item.replacement_order_id):[];
 const store=db.prepare('SELECT about_title,address,city,state,pincode,email,phone,logo_data FROM store_profile WHERE id=1').get()||{};
...
app.delete("/api/admin/products/:id",auth,admin,(req,res)=>{try{const id=Number(req.params.id);if(!Number.isSafeInteger(id)||id<1)return res.status(400).json({error:'Invalid product ID'});const product=permanentlyDeleteProduct(id);if(!product)return res.status(404).json({error:'Product not found'});logAdminActivity(req,'PRODUCT_DELETED','PRODUCT',product.id,{name:product.name,stock:product.stock,price:product.price,permanent:true});publishCatalogUpdate('deleted',product.id);res.json({ok:true})}catch(e){console.error('[Permanent product delete]',e);res.status(500).json({error:'Product could not be deleted. Please refresh inventory and check again.'})}});
...
app.post('/api/webhooks/courier-status',async(req,res)=>{
 try{
  ...
  const a=Buffer.from(expected),b=Buffer.from(supplied);if(a.length!==b.length||!crypto.timingSafeEqual(a,b))return res.status(401).json({error:'Invalid webhook token'});
  const payload=req.body&&typeof req.body==='object'?req.body:{},awb=String(payload.awb||payload.awb_code||payload.tracking_number||'').trim(),shipmentId=String(payload.shipment_id||payload.sr_shipment_id||'').trim(),srOrderId=String(payload.sr_order_id||payload.shiprocket_order_id||'').trim(),rawStatus=String(payload.current_status||payload.shipment_status||payload.status||'').trim(),normalized=rawStatus.toUpperCase().replace(/[^A-Z0-9]+/g,'_').replace(/^_|_$/g,'');
  // Shiprocket's "Test Webhook" request may not include a real shipment.
  // Acknowledge that connectivity probe after token verification without
  ...
  const returnEventHash=crypto.createHash('sha256').update(JSON.stringify(payload)).digest('hex');if(applyReturnWebhook(db,payload,returnEventHash,addReturnEvent))return res.json({ok:true,return_shipment:true});
  const eventHash=crypto.createHash('sha256').update(JSON.stringify(payload)).digest('hex');if(db.prepare('SELECT event_hash FROM shiprocket_webhook_events WHERE event_hash=?').get(eventHash))return res.json({ok:true,duplicate:true});
  ...
  if(payload.pickup_scheduled_date)db.prepare("UPDATE orders SET pickup_state='CONFIRMED',pickup_confirmed_date=? WHERE id=?").run(String(payload.pickup_scheduled_date).slice(0,100),order.id);
   if(canAdvance)addOrderEvent(order.id,order.user_id,target,`Order ${target.replaceAll('_',' ')}`,`Courier update: Order #${order.id} is now ${target.replaceAll('_',' ')}.`);
   db.prepare('INSERT INTO shiprocket_webhook_events(event_hash,awb,shipment_status) VALUES(?,?,?)').run(eventHash,awb,normalized);
  })();
  if(canAdvance){const label=target.replaceAll('_',' '),message=`Courier update: Order #${order.id} is now ${label}.`;const customer=db.prepare('SELECT name,email FROM users WHERE id=?').get(order.user_id);if(customer?.email)await notifyEmail(customer.email,`Ashwini Clothing Order #${order.id} - ${label}`,`Hello ${customer.name||'Customer'},\n\n${message}\n\nTrack your order from My Orders.`)}
 ...
 }catch(e){console.error('[Shiprocket webhook]',e.message);res.status(500).json({error:'Shiprocket webhook processing failed'})}
});
app.get("/api/webhooks/health",auth,admin,(req,res)=>res.json({ok:true,razorpayConfigured:Boolean(razorpay),shiprocketConfigured:shiprocketConfigured(),shiprocketWebhookConfigured:Boolean(String(process.env.SHIPROCKET_WEBHOOK_TOKEN||'').trim()),shiprocketPickupLocation:String(process.env.SHIPROCKET_PICKUP_LOCATION||'Home')}));
