...
app.patch('/api/admin/returns/:id',auth,admin,async(req,res)=>{try{
 ...
 if(['REQUESTING','UNKNOWN','REVIEW'].includes(r.sr_booking_state))return res.status(409).json({error:'Resolve this return booking in Shiprocket before changing its status.'});
...
function customerOrderView(order){const {courier_name,tracking_number,tracking_url,shiprocket_awb,shiprocket_courier_id,shiprocket_order_id,shiprocket_shipment_id,shiprocket_label_url,...customerOrder}=order;return customerOrder}
