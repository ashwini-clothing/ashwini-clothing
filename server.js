...
function saveDeliveryAddressFromOrder(userId,order){db.prepare(`INSERT INTO saved_delivery_addresses(user_id,name,phone,address_line,city,state,pincode,updated_at) VALUES(?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET name=excluded.name,phone=excluded.phone,address_line=excluded.address_line,city=excluded.city,state=excluded.state,pincode=excluded.pincode,updated_at=CURRENT_TIMESTAMP`).run(userId,order.delivery_name,order.customer_phone,order.delivery_address_line,order.delivery_city,order.delivery_state,order.delivery_pincode)}
...
app.get('/api/me/delivery-address',auth,(req,res)=>res.json({address:db.prepare('SELECT name,phone,address_line,city,state,pincode,updated_at FROM saved_delivery_addresses WHERE user_id=?').get(req.user.id)||null}));
app.put('/api/me/delivery-address',auth,async(req,res)=>{try{const name=String(req.body?.name||'').trim(),phone=String(req.body?.phone||'').replace(/\D/g,''),addressLine=String(req.body?.address_line||'').trim(),city=String(req.body?.city||'').trim(),state=String(req.body?.state||'').trim(),pincode=String(req.body?.pincode||'').trim();if(name.length<2||name.length>100)throw Error('Enter the customer full name');if(!/^[6-9]\d{9}$/.test(phone))throw Error('Enter a valid 10-digit Indian mobile number');if(addressLine.length<8||addressLine.length>500)throw Error('Enter a complete delivery address');if(!/^\d{6}$/.test(pincode))throw Error('Enter a valid 6-digit PIN code');const place=await lookupPostalPin(pincode);if(!place)throw Error('PIN code location could not be verified');const norm=v=>String(v||'').trim().toLowerCase().replace(/[^a-z0-9]+/g,' '),validCities=[place.city,place.district].map(norm).filter(Boolean);if(!validCities.includes(norm(city))||norm(state)!==norm(place.state))throw Error('City or state does not match the PIN code');db.prepare(`INSERT INTO saved_delivery_addresses(user_id,name,phone,address_line,city,state,pincode,updated_at) VALUES(?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET name=excluded.name,phone=excluded.phone,address_line=excluded.address_line,city=excluded.city,state=excluded.state,pincode=excluded.pincode,updated_at=CURRENT_TIMESTAMP`).run(req.user.id,name,phone,addressLine,city,state,pincode);res.json({ok:true,address:db.prepare('SELECT name,phone,address_line,city,state,pincode,updated_at FROM saved_delivery_addresses WHERE user_id=?').get(req.user.id)})}catch(e){res.status(400).json({error:e.message})}});
