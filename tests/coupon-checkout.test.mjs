...
function setup(existing=false){
 ...
 vm.createContext(c);for(const name of ['selectCheckoutCoupon','editCheckoutCoupon','removeCheckoutCoupon'])vm.runInContext(frontend.split('\n').find(line=>line.startsWith('function '+name+'(')),c);vm.runInContext(frontend.split('\n').find(x=>x.startsWith('async function applyCoupon(')),c);
...
test('every tap checks configured code, including ineligible customers',async()=>{const x=setup(true);x.nodes.coupon.value='';x.nodes.coupon.dataset={defaultCode:'NEW2026'};await x.c.selectCheckoutCoupon(1000);assert.match(x.nodes.couponMsg.textContent,/not available/);assert.equal(x.nodes.coupon.value,'');await x.c.selectCheckoutCoupon(1000);assert.match(x.nodes.couponMsg.textContent,/not available/);});
...
test('no configuration gives visible feedback',()=>{const x=setup();x.nodes.coupon.value='';x.nodes.coupon.dataset={defaultCode:''};x.c.selectCheckoutCoupon(1000);assert.match(x.nodes.couponMsg.textContent,/No active/);});
test('remove cancels pending responses and leaves purchase undiscounted',async()=>{const x=checkoutContext();let resolve;const api=x.c.api;x.c.api=()=>new Promise(r=>resolve=r);const pending=x.c.applyCoupon(1000);x.c.removeCheckoutCoupon(1000);resolve({code:'NEW2026',discount_percent:30});await pending;assert.equal(x.nodes.coupon.value,'');assert.match(x.nodes.couponMsg.textContent,/No coupon applied/);x.c.api=api;await x.c.pay();assert.equal(x.c.window.__checkoutReview.total,1000);});
test('editing invalidates pending coupon result before blur',async()=>{const x=setup();let resolve;x.c.api=()=>new Promise(r=>resolve=r);const pending=x.c.applyCoupon(1000);x.nodes.coupon.value='SAVE10';x.c.editCheckoutCoupon(1000);resolve({code:'NEW2026',discount_percent:30});await pending;assert.match(x.nodes.couponMsg.textContent,/Tap Apply/);assert.match(x.nodes.checkoutTotal.textContent,/1,000/);});
